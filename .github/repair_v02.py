...
# This helper is idempotent. It applies only the narrow V0.2 runtime/ledger
# changes that were audited against production history and then exits.

ps = ROOT / 'fronted/paper_state.py'
s = ps.read_text()
s = s.replace('CYCLE_INTERVAL_MS = 60_000', 'CYCLE_INTERVAL_MS = 5_000')
ps.write_text(s)
...
old_prefer = '"Prefer": "return=representation"'
new_prefer = '"Prefer": "resolution=merge-duplicates,return=representation" if "on_conflict=" in query else "return=representation"'
if old_prefer in s and 'resolution=merge-duplicates' not in s:
    s = s.replace(old_prefer, new_prefer, 1)
s = s.replace('saved = await _supabase(env, "market_observations", payload=payload)', 'saved = await _supabase(env, "market_observations", query="?on_conflict=symbol,minute_bucket", payload=payload)')

marker = '\n\ndef _trade_ts(point):\n'
if '_persist_execution_trade' not in s:
    helper = r'''
...
if mig.exists():
    ...
    mig.write_text('''-- V0.2 execution ledger reconciliation.\nALTER TABLE public.decisions ADD COLUMN IF NOT EXISTS trade_id bigint;\nALTER TABLE public.trades ADD COLUMN IF NOT EXISTS exit_decision_id bigint;\nDO $$ BEGIN\n  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'decisions_trade_id_fkey') THEN ALTER TABLE public.decisions ADD CONSTRAINT decisions_trade_id_fkey FOREIGN KEY (trade_id) REFERENCES public.trades(id); END IF;\n  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'trades_exit_decision_id_fkey') THEN ALTER TABLE public.trades ADD CONSTRAINT trades_exit_decision_id_fkey FOREIGN KEY (exit_decision_id) REFERENCES public.decisions(id); END IF;\nEND $$;\nCREATE INDEX IF NOT EXISTS idx_trades_open_symbol ON public.trades(symbol, status, created_at DESC);\nCREATE INDEX IF NOT EXISTS idx_decisions_trade_id ON public.decisions(trade_id);\nCREATE UNIQUE INDEX IF NOT EXISTS uq_market_observations_symbol_minute ON public.market_observations(symbol, minute_bucket);\n''')
