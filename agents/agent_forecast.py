...
Forecast is independent evidence, not a second technical voter. Horizons are
market bars, not days, so the agent can operate on the same 1m snapshot used by
scalping. Missing history is UNAVAILABLE, never a HOLD vote.
"""
...
class ForecastAgent:
    """Independent short-horizon forecaster based only on price/volatility history."""

    def __init__(self, config: Optional[Dict[str, Any]] = None):
        cfg = config or {}
        self.horizons = (
            int(cfg.get("short_bars", 3)),
            int(cfg.get("medium_bars", 8)),
            int(cfg.get("long_bars", 15)),
        )
    ...
    def analyze(self, symbol: str, sentiment_result: Any = None, technical_result: Any = None,
        ...
        short = self._trend_return(closes, min(3, len(closes) - 1))
        medium = self._trend_return(closes, min(8, len(closes) - 1))
        long = self._trend_return(closes, min(15, len(closes) - 1))
        ...
        normalized = drift / max(volatility * np.sqrt(max(self.horizons[0], 1)), 0.001)
        score = float(np.clip(np.tanh(normalized), -1, 1))
        directions = [1 if x > 0 else -1 if x < 0 else 0 for x in (short, medium, long)]
        directional = [x for x in directions if x]
        agreement = abs(sum(directional)) / len(directional) if directional else 0.0
        confidence = float(np.clip(0.42 + 0.28 * abs(score) + 0.20 * agreement + 0.10 * min(len(closes) / 150, 1.0), 0.35, 0.86))
        direction = "BULLISH" if score >= 0.15 else "BEARISH" if score <= -0.15 else "NEUTRAL"
        probs = self._probabilities(score)
        evidence = [
            f"short={short:+.3%}", f"medium={medium:+.3%}", f"long={long:+.3%}",
            f"volatility={volatility:.3%}", f"horizon_agreement={agreement:.0%}",
            f"UP={probs['UP']:.0%}", f"DOWN={probs['DOWN']:.0%}",
        ]
        ...
        return ForecastResult(
            ...
            expected_high=max(paths["long"]) * (1 + volatility),
            expected_low=min(paths["long"]) * (1 - volatility),
            ...
            recommendations=["Forecast is independent probabilistic evidence; Trader owns action."],
            model_predictions=paths, model_weights={"short": 0.55, "medium": 0.30, "long": 0.15},
            model_status={"short_horizon": "OK"}, data_quality={"history": min(len(closes) / 150, 1.0)},
            model_agreement=agreement, forecast_score=score, forecast_action=action, warnings=[], direction=direction,
            ...
            limitations=["Forecast is probabilistic and independent of the technical vote; it is not an execution signal."],
        )
    ...
    def _path(price, drift, bars):
        ...
        return [price * ((1 + drift) ** i) for i in range(1, bars + 1)]

    def _prediction(self, price, path, vol, horizon):
        pred = float(path[-1] if path else price)
        change = (pred / price - 1) * 100
        change = float(np.clip(change, -self.max_move * 100, self.max_move * 100))
        pred = price * (1 + change / 100)
        width = price * vol * np.sqrt(max(len(path), 1)) * 1.5
        conf = float(np.clip(0.45 + min(abs(change) / max(vol * 100, 0.1), 2) * 0.12, 0.35, 0.75))
        return PricePrediction(datetime.now(timezone.utc) + timedelta(minutes=max(1, len(path))), pred,
                               max(0, pred - width), pred + width, conf, horizon, change)

    @staticmethod
    def _probabilities(score):
        up = max(0.02, 0.5 + score * 0.35)
        down = max(0.02, 0.5 - score * 0.35)
        side = max(0.02, 1.0 - abs(score))
        total = up + down + side
        return {"UP": up / total, "DOWN": down / total, "SIDEWAYS": side / total}
    ...
    def _unavailable(self, symbol, price, timeframe, reason, age, status=SignalStatus.UNAVAILABLE.value):
        ...
        return ForecastResult(symbol, datetime.now(timezone.utc), price, p, p, p, [], [], [], {}, "NEUTRAL", 0.0,
                              {"UP": 0.0, "DOWN": 0.0, "SIDEWAYS": 1.0}, price, price, {"high": price, "low": price},
                              [], [], f"Forecast unavailable: {reason}", [], {}, {}, {}, 0.0, 0.0, "HOLD", [reason],
                              direction="NEUTRAL", score=0.0, confidence=0.0, timeframe=timeframe, evidence=[reason],
                              data_age_seconds=age, status=status, limitations=[reason])
