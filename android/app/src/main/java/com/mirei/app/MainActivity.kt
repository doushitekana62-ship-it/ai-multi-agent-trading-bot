...
class MainActivity : Activity() {
    ...
    private val receiver = object : BroadcastReceiver() { override fun onReceive(context: Context?, intent: Intent?) { if (intent?.action != MireiForegroundService.ACTION_STATUS) return; currentIntent = intent; renderHeader(); renderCurrentTab() } }
    private val clockRunnable = object : Runnable { override fun run() { renderHeader(); if (currentTab == Tab.RINGKASAN) renderCurrentTab(); handler.postDelayed(this, 1_000L) } }

    override fun onCreate(savedInstanceState: Bundle?) { super.onCreate(savedInstanceState); PositionTradeConfigStore.reload(this); requestNotificationPermissionIfNeeded(); buildShell(); requestRefresh(); handler.post(clockRunnable) }
    ...
    private fun buildShell() {
        ...
        shell.addView(text("Mirei", 30f, true)); shell.addView(text("Asisten trading · lokal · paper trading", 14f), margin(0, 0, 0, 6))
        statusCard = card("BERHENTI · PAPER ONLY"); shell.addView(statusCard, margin(0, 0, 0, 6))
        tabsLayout = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER }; shell.addView(tabsLayout, margin(0, 0, 0, 6))
        content = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(0, 0, 0, 16) }
        val scroll = ScrollView(this).apply { isFillViewport = true; addView(content, ViewGroup.LayoutParams(-1, -2)) }; shell.addView(scroll, LinearLayout.LayoutParams(-1, 0, 1f))
        setContentView(shell); renderTabs(); renderCurrentTab()
    }

    private fun renderTabs() {
        val tabs = tabsLayout ?: return; tabs.removeAllViews()
        Tab.values().forEach { tab -> val b = button(tab.label) { currentTab = tab; renderTabs(); renderCurrentTab() }; b.textSize = 11f; b.isAllCaps = false; if (tab == currentTab) b.setBackgroundColor(Color.rgb(88, 88, 88)); tabs.addView(b, LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f).apply { rightMargin = 3 }) }
    }
    ...
    private fun renderCurrentTab() {
        content.removeAllViews(); runCatching { when (currentTab) { Tab.RINGKASAN -> renderSummary(); Tab.POSISI -> renderPositions(); Tab.PASAR -> renderMarket(); Tab.AUDIT -> renderAudit() } }.onFailure { content.addView(card("UI error: ${it.message ?: "unknown"}")) }
    }

    private fun renderSummary() {
        addTitle("DASHBOARD · RINGKASAN"); val i = currentIntent
        if (i == null) { content.addView(card("Belum ada snapshot runtime.")); addControls(); return }
        val positions = i.getIntExtra(MireiForegroundService.EXTRA_POSITIONS, 0); val buys = i.getIntExtra(MireiForegroundService.EXTRA_BUY_COUNT, 0); val holds = i.getIntExtra(MireiForegroundService.EXTRA_HOLD_COUNT, 0); val sells = i.getIntExtra(MireiForegroundService.EXTRA_SELL_COUNT, 0); val profiles = PositionTradeConfigStore.snapshot()
        ...
        addTitle("AI VS ACTUAL · SESI INI"); content.addView(card("AI DECISION  · BUY $buys · HOLD $holds · SELL $sells\nACTUAL PAPER · posisi aktif $positions\n\nAI OPEN adalah keputusan analisis. ACTUAL OPEN hanya bertambah setelah execution gate benar-benar berhasil.", 13f))
        addTitle("PENGATURAN AKTIF")
        if (profiles.isEmpty()) content.addView(card("Belum ada kontrak instrument tersimpan.")) else profiles.forEach { (symbol, profile) -> content.addView(card("$symbol\nMode: ${profile.mode?.name ?: "BALANCED"}\nTP/SL: ${if (profile.manualRiskMode.name == "MANUAL") "MANUAL" else "AUTO"}\nTP: ${profile.manualNetProfitTargetIdr?.let { "NET Rp${numberFormat.format(it)}" } ?: profile.manualTakeProfitPercent?.let { "${it}%" } ?: "mengikuti mode"}\nSL: ${profile.stopLossPercent?.let { "${it}%" } ?: "mengikuti mode"}\nDasar: ${profile.riskReferenceMode?.let { riskBasisLabel(it.name) } ?: "HARGA ENTRY"}", 12.5f)) }
        addControls()
    }

    private fun addControls() {
        addTitle("KONTROL SESI")
        val row1 = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL }; row1.addView(button("MULAI") { showStartDialog() }, weight()); row1.addView(button("LANJUTKAN") { send(MireiForegroundService.ACTION_START) }, weight()); row1.addView(button("SEGARKAN DATA") { requestRefresh() }, weight()); content.addView(row1)
        val row2 = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL }; row2.addView(button("JEDA / HOLD") { send(MireiForegroundService.ACTION_HOLD) }, weight()); row2.addView(button("BERHENTI") { send(MireiForegroundService.ACTION_STOP) }, weight()); content.addView(row2)
        content.addView(button("TUTUP SEMUA POSISI") { send(MireiForegroundService.ACTION_CLOSE_ALL) }); content.addView(button("PENGATURAN / MARKET SELECTION") { showStartDialog() })
    }

    private fun renderPositions() {
        addTitle("POSISI AKTIF"); val rows = parsePositions(currentIntent?.getStringExtra(MireiForegroundService.EXTRA_POSITIONS_DETAIL).orEmpty()); if (rows.isEmpty()) { content.addView(card("Tidak ada posisi aktif.")); return }
        rows.forEach { row -> val symbol = row["symbol"].orEmpty(); val profile = PositionTradeConfigStore.get(symbol); val tpMode = profile?.manualNetProfitTargetIdr?.let { "MANUAL NET Rp${numberFormat.format(it)}" } ?: profile?.manualTakeProfitPercent?.let { "MANUAL ${it}%" } ?: "AUTO/MODE"; content.addView(card("$symbol\nModal Rp ${numberFormat.format(row["stake"].orEmpty().toDoubleOrNull() ?: 0.0)}\nEntry Rp ${numberFormat.format(row["entry"].orEmpty().toDoubleOrNull() ?: 0.0)}\nSekarang Rp ${numberFormat.format(row["current"].orEmpty().toDoubleOrNull() ?: 0.0)}\nPnL berjalan Rp ${signedMoney(row["unrealized"].orEmpty().toDoubleOrNull() ?: 0.0)}\n\nTP Rp ${numberFormat.format(row["tp"].orEmpty().toDoubleOrNull() ?: 0.0)} · $tpMode\nSL Rp ${numberFormat.format(row["sl"].orEmpty().toDoubleOrNull() ?: 0.0)}\nDasar: ${riskBasisLabel(row["risk_basis"].orEmpty())}\nAsal: ${row["entry_reason"].orEmpty()}", 13f)) }
    }

    private fun renderMarket() {
        addTitle("PASAR"); val i = currentIntent ?: run { content.addView(card("Belum ada snapshot pasar.")); return }; val symbol = i.getStringExtra(MireiForegroundService.EXTRA_SYMBOL) ?: "BTC/IDR"
        ...
        addTitle("SCANNER"); val rows = i.getStringExtra(MireiForegroundService.EXTRA_SCANNER).orEmpty().lines().filter { it.isNotBlank() }.take(12); if (rows.isEmpty()) content.addView(card("Belum ada hasil scanner.")); rows.forEach { line -> val p = line.split('|'); if (p.size >= 8) content.addView(card("${p[0]} · ${p[1]} · provider ${p[2]}\nHarga ${p[3]} · 1M ${p[4]}% · Momentum ${p[5]}% · Trend ${p[6]}% · Volume share ${p[7]}%", 12f)) }
    }

    private fun renderAudit() {
        addTitle("LOG AUDIT · SESI INI"); content.addView(card("Waktu | Simbol | Event | Harga/PnL | Alasan", 11f)); val start = currentIntent?.getLongExtra(MireiForegroundService.EXTRA_SESSION_CREATED, 0L) ?: 0L
        runCatching {
            val db = MireiDatabase(this); val trades = db.recentTrades(150).filter { start == 0L || it.openedAtEpochMs >= start }; val audits = db.recentAudit(250).filter { start == 0L || it.createdAtEpochMs >= start }
            ...
            trades.forEach { trade -> addAuditRow(table, formatEpoch(trade.openedAtEpochMs), trade.symbol, trade.status, trade.exitPrice?.let { "Rp ${numberFormat.format(it)}" } ?: "Rp ${numberFormat.format(trade.entryPrice ?: 0.0)} / PnL ${signedMoney(trade.pnlIdr)}", trade.entryReason) }
            audits.forEach { event -> val details = event.details.replace('|', ' '); val symbol = details.substringAfter("symbol=", "—").substringBefore(' '); val pnl = details.substringAfter("pnl=", "").substringBefore(' '); val priceText = if (pnl.isNotBlank()) "PnL $pnl" else "—"; addAuditRow(table, formatEpoch(event.createdAtEpochMs), symbol, event.eventType, priceText, details) }
    ...
    private fun addAuditRow(table: LinearLayout, time: String, symbol: String, event: String, price: String, reason: String) { val row = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; setBackgroundColor(Color.rgb(24, 34, 43)) }; listOf(time, symbol, event, price, reason).forEach { value -> row.addView(text(value, 10f), LinearLayout.LayoutParams(dp(118), ViewGroup.LayoutParams.WRAP_CONTENT)) }; table.addView(row, margin(0, 1, 0, 1)) }
