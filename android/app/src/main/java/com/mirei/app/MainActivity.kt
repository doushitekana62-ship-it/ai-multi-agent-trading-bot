...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState); requestNotificationPermissionIfNeeded(); numberFormat.maximumFractionDigits = 2; buildStopScreen()
    }
    override fun onStart() {
        super.onStart(); val filter = IntentFilter(MireiForegroundService.ACTION_STATUS); if (Build.VERSION.SDK_INT >= 33) registerReceiver(statusReceiver, filter, RECEIVER_NOT_EXPORTED) else registerReceiver(statusReceiver, filter)
    }
    override fun onStop() { runCatching { unregisterReceiver(statusReceiver) }; super.onStop() }

    private fun buildStopScreen() {
        screenRunning = false
        val content = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(28, 36, 28, 28) }
        content.addView(title("Mirei ミレイ", 30f)); content.addView(title("Android-first paper trading runtime", 16f))
        content.addView(card("PAPER BOOTSTRAP\nStarting capital: Rp 150.000\nAfter START, enter the coins and IDR amounts you already hold.\nMirei never creates a BUY to consume the starting capital."), marginParams(0, 22, 0, 12))
        status = TextView(this).apply { textSize = 16f; text = "STATE\nState: STOP\nExecution: PAPER ONLY\nPress START to define the initial portfolio." }
        content.addView(status, marginParams(0, 8, 0, 12)); content.addView(actionButton("START") { showStartDialog() }); content.addView(actionButton("REFRESH") { sendSimpleAction(MireiForegroundService.ACTION_REFRESH) }); content.addView(actionButton("DELETE HISTORY") { confirmDeleteHistory() })
        dashboard = content; setContentView(scroll(content))
    }
    ...
    private fun buildRunningScreen() {
        ...
        content.addView(title("Mirei ミレイ", 28f)); content.addView(title("Live paper telemetry · runtime cycle 5s", 14f))
        status = TextView(this).apply { textSize = 15f; setLineSpacing(0f, 1.04f) }; content.addView(status, marginParams(0, 18, 0, 10))
        marketChart = SparklineView(this); content.addView(cardView("MARKET PULSE / PRICE GRAPH", marketChart, 220))
        content.addView(actionButton("RISK / EXIT SETTINGS") { showRiskDialog() }); content.addView(actionButton("REFRESH DATA") { sendSimpleAction(MireiForegroundService.ACTION_REFRESH) }); content.addView(actionButton("HOLD") { sendSimpleAction(MireiForegroundService.ACTION_HOLD) }); content.addView(actionButton("CLOSE ALL") { sendSimpleAction(MireiForegroundService.ACTION_CLOSE_ALL) }); content.addView(actionButton("STOP") { sendSimpleAction(MireiForegroundService.ACTION_STOP) }); content.addView(actionButton("DELETE HISTORY") { confirmDeleteHistory() })
        historyContainer = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL }; content.addView(historyContainer, marginParams(0, 18, 0, 0)); dashboard = content; setContentView(scroll(content))
    }
    ...
    private fun showStartDialog() {
        ...
        repeat(3) { _ ->
            val row = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL }; val coin = spinner(MireiForegroundService.SUPPORTED_MARKETS); val amount = moneyEdit("0")
            coinSpinners += coin; amounts += amount; row.addView(coin, LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1.1f)); row.addView(amount, LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 0.9f)); root.addView(row)
        }
        root.addView(title("RISK PROFILE", 14f), marginParams(0, 14, 0, 3)); val modes = listOf("BALANCED", "AGGRESSIVE", "SAFETY"); val modeSpinner = spinner(modes); root.addView(modeSpinner)
        val manual = CheckBox(this).apply { text = "Manual TP/SL — locks profile" }; root.addView(manual); val sl = percentEdit("0.50"); val tp = percentEdit("1.00"); root.addView(title("Stop Loss %", 12f)); root.addView(sl); root.addView(title("Take Profit %", 12f)); root.addView(tp)
        ...
        AlertDialog.Builder(this).setTitle("START PAPER RUNTIME").setView(root).setNegativeButton("CANCEL", null).setPositiveButton("START") { _, _ ->
            val allocations = parseAllocations(coinSpinners, amounts); val total = allocations.values.sum()
            if (allocations.isEmpty() || allocations.size > 3 || total <= 0.0 || total > 150000.0001) { showMessage("Invalid portfolio", "Enter 1–3 coins and allocate between Rp 1 and Rp 150.000 total."); return@setPositiveButton }
            if (manual.isChecked) { val slValue = sl.text.toString().replace(",", ".").toDoubleOrNull(); val tpValue = tp.text.toString().replace(",", ".").toDoubleOrNull(); if (slValue == null || tpValue == null || slValue <= 0.0 || tpValue <= slValue) { showMessage("Invalid TP/SL", "Take Profit must be greater than Stop Loss."); return@setPositiveButton } }
            val selectedExchange = MireiForegroundService.SUPPORTED_EXCHANGES[exchangeSpinner.selectedItemPosition]
            if (selectedExchange != MireiForegroundService.DEFAULT_EXCHANGE) { showMessage("Exchange adapter not active", "${exchangeLabel(selectedExchange)} is catalogued for Freqtrade-style exchange integration, but this Android paper adapter currently uses INDODAX public market data."); return@setPositiveButton }
            persistRisk(modeSpinner.selectedItem.toString(), manual.isChecked, sl.text.toString(), tp.text.toString()); startRuntime(allocations.entries.joinToString(";") { "${it.key}=${it.value}" }, allocations.keys.first(), selectedExchange)
        }.show()
    }

    private fun startRuntime(raw: String, primary: String, exchange: String) { val intent = Intent(this, MireiForegroundService::class.java).setAction(MireiForegroundService.ACTION_START).apply { putExtra(MireiForegroundService.EXTRA_SYMBOL, primary); putExtra(MireiForegroundService.EXTRA_EXCHANGE, exchange); putExtra(MireiForegroundService.EXTRA_INITIAL_ALLOCATIONS, raw) }; if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) startForegroundService(intent) else startService(intent); buildRunningScreen() }
    private fun parseAllocations(coins: List<Spinner>, amounts: List<EditText>): LinkedHashMap<String, Double> = linkedMapOf<String, Double>().apply { for (i in coins.indices) { val amount = amounts[i].text.toString().replace(",", ".").toDoubleOrNull() ?: 0.0; if (amount > 0.0) put(coins[i].selectedItem.toString(), amount) } }

    private fun showRiskDialog() {
        val modes = listOf("BALANCED", "AGGRESSIVE", "SAFETY"); val root = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(20, 6, 20, 4) }; val mode = spinner(modes); mode.setSelection(max(0, modes.indexOf(prefs.getString(KEY_MODE, "BALANCED")))); root.addView(title("Risk profile", 14f)); root.addView(mode)
        val manual = CheckBox(this).apply { text = "Manual TP/SL — locks profile" }; root.addView(manual); val sl = percentEdit(prefs.getString(KEY_MANUAL_SL, "0.50") ?: "0.50"); val tp = percentEdit(prefs.getString(KEY_MANUAL_TP, "1.00") ?: "1.00"); root.addView(title("Stop Loss %", 12f)); root.addView(sl); root.addView(title("Take Profit %", 12f)); root.addView(tp)
        ...
        AlertDialog.Builder(this).setTitle("AI RISK & EXIT PROFILE").setView(root).setNegativeButton("CANCEL", null).setPositiveButton("SAVE") { _, _ ->
            val slValue = sl.text.toString().replace(",", ".").toDoubleOrNull(); val tpValue = tp.text.toString().replace(",", ".").toDoubleOrNull(); if (manual.isChecked && (slValue == null || tpValue == null || slValue <= 0.0 || tpValue <= slValue)) { showMessage("Invalid TP/SL", "Take Profit must be greater than Stop Loss."); return@setPositiveButton }; persistRisk(mode.selectedItem.toString(), manual.isChecked, slValue?.toString() ?: "0.50", tpValue?.toString() ?: "1.00"); sendSimpleAction(MireiForegroundService.ACTION_APPLY_RISK)
        }.show()
    }
    ...
    private fun moneyEdit(value: String): EditText = EditText(this).apply { inputType = InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_FLAG_DECIMAL; hint = "Rp"; setText(value) }
    private fun percentEdit(value: String): EditText = EditText(this).apply { inputType = InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_FLAG_DECIMAL; setText(value) }
    ...
    private fun renderStatus(intent: Intent) {
        val state = intent.getStringExtra(MireiForegroundService.EXTRA_STATE) ?: "UNKNOWN"; if (state == "RUNNING" && !screenRunning) buildRunningScreen(); if (state == "STOP" && screenRunning) buildStopScreen()
        val symbol = intent.getStringExtra(MireiForegroundService.EXTRA_SYMBOL) ?: MireiForegroundService.DEFAULT_SYMBOL; val exchange = intent.getStringExtra(MireiForegroundService.EXTRA_EXCHANGE) ?: MireiForegroundService.DEFAULT_EXCHANGE; val price = intent.getDoubleExtra(MireiForegroundService.EXTRA_PRICE, 0.0); val bid = intent.getDoubleExtra(MireiForegroundService.EXTRA_BID, 0.0); val ask = intent.getDoubleExtra(MireiForegroundService.EXTRA_ASK, 0.0); val high = intent.getDoubleExtra(MireiForegroundService.EXTRA_HIGH_24H, 0.0); val low = intent.getDoubleExtra(MireiForegroundService.EXTRA_LOW_24H, 0.0); val volume = intent.getDoubleExtra(MireiForegroundService.EXTRA_VOLUME_24H, 0.0)
        val equity = intent.getDoubleExtra(MireiForegroundService.EXTRA_EQUITY, 0.0); val balance = intent.getDoubleExtra(MireiForegroundService.EXTRA_BALANCE, 0.0); val pnl = intent.getDoubleExtra(MireiForegroundService.EXTRA_PNL, 0.0); val positions = intent.getIntExtra(MireiForegroundService.EXTRA_POSITIONS, 0); val confidence = intent.getDoubleExtra(MireiForegroundService.EXTRA_CONFIDENCE, 0.0); val action = intent.getStringExtra(MireiForegroundService.EXTRA_ACTION) ?: "HOLD"; val rationale = intent.getStringExtra(MireiForegroundService.EXTRA_RATIONALE) ?: "no_decision"; val agents = intent.getStringExtra(MireiForegroundService.EXTRA_AGENT_SUMMARY).orEmpty(); val reasons = intent.getStringExtra(MireiForegroundService.EXTRA_ENTRY_REASONS).orEmpty()
    ...
    private fun refreshHistoryViews() {
        ...
        historyContainer.removeAllViews(); val db = MireiDatabase(this); historyContainer.addView(historySection("PAPER HISTORY — TRADES", tradesText(db.recentTrades(20)))); historyContainer.addView(historySection("DECISION HISTORY — LATEST 30", decisionsText(db.recentSuggestions(30))))
    }
    private fun tradesText(rows: List<TradeRow>): String = if (rows.isEmpty()) "No trade history." else rows.joinToString("\n") { "${formatEpoch(it.closedAtEpochMs ?: it.openedAtEpochMs)} | ${it.symbol} | ${it.side} | ${it.status} | Rp ${numberFormat.format(it.stakeIdr)} | entry ${numberFormat.format(it.entryPrice ?: 0.0)} | exit ${numberFormat.format(it.exitPrice ?: 0.0)} | PnL Rp ${numberFormat.format(it.pnlIdr)} | ${it.exitReason ?: "OPEN"}" }
