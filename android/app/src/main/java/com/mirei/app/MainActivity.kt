...
class MainActivity : Activity() {
    ...
    private fun renderHeader() {
        ...
        val state = stateLabel(i.getStringExtra(MireiForegroundService.EXTRA_STATE) ?: "STOP")
        val exchange = i.getStringExtra(MireiForegroundService.EXTRA_EXCHANGE)?.uppercase(Locale.US) ?: "INDODAX"
    ...
    private fun addControls() {
        ...
        row1.addView(button("MULAI") { startOrExplain() }, weight())
        ...
        content.addView(button("TERAPKAN PERUBAHAN RISIKO") { send(MireiForegroundService.ACTION_APPLY_RISK) })
        content.addView(button("VERIFIKASI MANUAL / RE-ENTRY") { showHumanVerifyDialog() })
        val admin = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL }
        admin.addView(button("RESET SESI") { confirmResetSession() }, weight())
        admin.addView(button("RESET JAM") { confirmResetClock() }, weight())
        admin.addView(button("HAPUS RIWAYAT") { confirmDeleteHistory() }, weight())
        content.addView(admin)
        content.addView(button("PENGATURAN / MARKET SELECTION") { startOrExplain(openSettings = true) })
    }

    private fun startOrExplain(openSettings: Boolean = false) {
        val state = currentIntent?.getStringExtra(MireiForegroundService.EXTRA_STATE)
        val active = state == "RUNNING" || state == "HOLD"
        if (active && !openSettings) {
            AlertDialog.Builder(this).setTitle("SESI SEDANG BERJALAN")
                .setMessage("MULAI tidak membuat sesi baru. Perintah ini melanjutkan sesi lama dengan konfigurasi runtime yang tersimpan. Untuk konfigurasi market/modal baru, buka PENGATURAN / MARKET SELECTION lalu TERAPKAN PERUBAHAN RISIKO, atau BERHENTI sebelum MULAI ulang.")
    ...
    private fun showHumanVerifyDialog() {
        val i = currentIntent
        if (i == null) { AlertDialog.Builder(this).setTitle("VERIFIKASI MANUAL").setMessage("Belum ada snapshot runtime.").setPositiveButton("OK", null).show(); return }
        val box = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(12, 4, 12, 4) }
        val symbol = EditText(this).apply { hint = "Simbol (contoh BTC/IDR)"; setSingleLine(true); setText(i.getStringExtra(MireiForegroundService.EXTRA_SYMBOL).orEmpty()) }
        val agents = EditText(this).apply { hint = "Agent yang diverifikasi, pisahkan koma"; setSingleLine(true); setText(i.getStringExtra(MireiForegroundService.EXTRA_HUMAN_ALLOWED).orEmpty()) }
        box.addView(symbol); box.addView(agents)
        AlertDialog.Builder(this).setTitle("VERIFIKASI MANUAL / RE-ENTRY").setMessage("Gunakan hanya setelah membaca indikator runtime. Ini tidak mengaktifkan live trading.").setView(box).setNegativeButton("BATAL", null).setPositiveButton("VERIFIKASI") { _, _ ->
            send(MireiForegroundService.ACTION_HUMAN_VERIFY) { putExtra(MireiForegroundService.EXTRA_HUMAN_SYMBOL, symbol.text.toString().trim()); putExtra(MireiForegroundService.EXTRA_HUMAN_AGENTS, agents.text.toString().trim()) }
        }.show()
    }

    private fun confirmResetSession() = AlertDialog.Builder(this).setTitle("RESET SESI?").setMessage("Reset state portfolio/session tanpa menghapus riwayat audit.").setNegativeButton("BATAL", null).setPositiveButton("RESET") { _, _ -> send(MireiForegroundService.ACTION_RESET_SESSION) }.show()
    private fun confirmResetClock() = AlertDialog.Builder(this).setTitle("RESET JAM SESI?").setMessage("Titik waktu pengujian akan diatur ulang ke sekarang.").setNegativeButton("BATAL", null).setPositiveButton("RESET") { _, _ -> send(MireiForegroundService.ACTION_RESET_CLOCK) }.show()
    private fun confirmDeleteHistory() = AlertDialog.Builder(this).setTitle("HAPUS RIWAYAT?").setMessage("Trade history dan audit history lokal akan dibersihkan. Sesi aktif tidak diubah.").setNegativeButton("BATAL", null).setPositiveButton("HAPUS") { _, _ -> send(MireiForegroundService.ACTION_DELETE_HISTORY) }.show()
    ...
    private fun send(action: String, extras: Intent.() -> Unit = {}) {
        ...
        }.onFailure { AlertDialog.Builder(this).setTitle("Mirei").setMessage(it.message ?: "Perintah gagal").setPositiveButton("OK", null).show() }
    }
