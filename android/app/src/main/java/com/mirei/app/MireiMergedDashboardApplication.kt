...
/** Merged presentation layer: MainActivity keeps ownership of the original page and controls. */
class MireiMergedDashboardApplication : Application() {
    ...
    private fun install(activity: MainActivity) {
        ...
        root.viewTreeObserver.addOnGlobalLayoutListener {
            ...
            if (currentIntent(activity) == null) return@addOnGlobalLayoutListener
    ...
    private fun mergeDashboard(activity: MainActivity) {
        ...
        val dashboard = LinearLayout(activity).apply { orientation = LinearLayout.VERTICAL; setTag(DASHBOARD_TAG) }
        ...
        dashboard.addView(card(activity, "Internet: ${if (intent.getBooleanExtra(MireiForegroundService.EXTRA_INTERNET, false)) "OK" else "PUTUS"}\nMarket: ${if (intent.getBooleanExtra(MireiForegroundService.EXTRA_MARKET_FRESH, false)) "SEGAR" else "STALE"}\nExchange: ${if (intent.getBooleanExtra(MireiForegroundService.EXTRA_EXCHANGE_HEALTHY, false)) "OK" else "TIDAK SIAP"}", 13f), params(5))
        content.addView(dashboard, minOf(2, content.childCount))
    }
    private fun hasDashboard(content: LinearLayout): Boolean = content.findViewWithTag<View>(DASHBOARD_TAG) != null
    ...
    private fun findMenu(view: View): Spinner? {
        if (view is Spinner && view.selectedItem?.toString() in setOf("RINGKASAN", "PASAR", "POSISI", "AKTIVITAS", "KEPUTUSAN", "RISIKO", "EXCHANGE / API", "PENGATURAN", "LOG / AUDIT")) return view
...
// Final validation marker: the merged dashboard must retain a visible MULAI action.
