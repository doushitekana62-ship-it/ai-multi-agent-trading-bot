...
class MireiUiPatchApplication : Application() {
    ...
    private fun renderMarketPulse(activity: MainActivity) {
        ...
        selector.setOnItemSelectedListener(object : android.widget.AdapterView.OnItemSelectedListener {
            ...
            override fun onItemSelected(parent: android.widget.AdapterView<*>?, view: View?, position: Int, id: Long) {
                symbols.getOrNull(position)?.let(::renderSelected)
            }
        })
    ...
    private fun renderSynchronizedSummary(activity: MainActivity) {
        ...
        if (lastClose != null) {
            ...
            content.addView(card(activity, "Belum ada CLOSE nyata pada sesi ini.", 13.5f))
        }
        if (lastOpen != null) {
            content.addView(card(activity,
        ...
        val reentryText = if (lastClose == null) {
            ...
            "RE-ENTRY terdeteksi $reEntriesx. Ini membuktikan modal hasil CLOSE dapat kembali masuk saat gate BUY valid."
        } else {
    ...
    private fun renderReadableAudit(activity: MainActivity) {
        ...
        if (executionTrades.isEmpty()) {
            content.addView(card(activity, "Belum ada transaksi nyata pada sesi ini.", 13f))
        } else {
            executionTrades.sortedByDescending { maxOf(it.openedAtEpochMs, it.closedAtEpochMs ?: 0L) }.take(100).forEach { trade ->
        ...
        events.take(100).forEach { event ->
            content.addView(card(activity,
                "${formatEpoch(event.createdAtEpochMs)} · ${humanEventType(event.eventType)}\n${humanAuditDetails(event.details)}", 12.5f))
        }
    ...
    private fun currentIntentOf(activity: MainActivity): Intent? = runCatching {
    ...
    private fun selectedFromCurrentIntent(intent: Intent) = com.mirei.app.core.MarketSnapshot(
        symbol = intent.getStringExtra(MireiForegroundService.EXTRA_SYMBOL) ?: "",
        price = intent.getDoubleExtra(MireiForegroundService.EXTRA_PRICE, 0.0),
        momentumPercent = intent.getDoubleExtra(MireiForegroundService.EXTRA_MOMENTUM, 0.0),
        volatilityPercent = intent.getDoubleExtra(MireiForegroundService.EXTRA_VOLATILITY, 0.0),
        sentimentScore = intent.getDoubleExtra(MireiForegroundService.EXTRA_SENTIMENT, 0.0),
        forecastConfidence = intent.getDoubleExtra(MireiForegroundService.EXTRA_FORECAST_CONFIDENCE, 0.0),
        dataFresh = intent.getBooleanExtra(MireiForegroundService.EXTRA_MARKET_FRESH, false),
        bidPrice = intent.getDoubleExtra(MireiForegroundService.EXTRA_BID, 0.0),
        askPrice = intent.getDoubleExtra(MireiForegroundService.EXTRA_ASK, 0.0),
        high24h = intent.getDoubleExtra(MireiForegroundService.EXTRA_HIGH_24H, 0.0),
        low24h = intent.getDoubleExtra(MireiForegroundService.EXTRA_LOW_24H, 0.0),
        volume24h = intent.getDoubleExtra(MireiForegroundService.EXTRA_VOLUME_24H, 0.0),
        spreadPercent = intent.getDoubleExtra(MireiForegroundService.EXTRA_SPREAD, 0.0),
        changeSinceLastTickPercent = intent.getDoubleExtra(MireiForegroundService.EXTRA_CHANGE_TICK, 0.0),
        change1mPercent = intent.getDoubleExtra(MireiForegroundService.EXTRA_CHANGE_1M, 0.0),
        change5mPercent = intent.getDoubleExtra(MireiForegroundService.EXTRA_CHANGE_5M, 0.0),
        change15mPercent = intent.getDoubleExtra(MireiForegroundService.EXTRA_CHANGE_15M, 0.0),
        tradeFlowPercent = intent.getDoubleExtra(MireiForegroundService.EXTRA_FLOW, 0.0),
        tradeCount = intent.getIntExtra(MireiForegroundService.EXTRA_TRADE_COUNT, 0),
        buyVolume = intent.getDoubleExtra(MireiForegroundService.EXTRA_BUY_VOLUME, 0.0),
        sellVolume = intent.getDoubleExtra(MireiForegroundService.EXTRA_SELL_VOLUME, 0.0),
        lastTradeEpochMs = intent.getLongExtra(MireiForegroundService.EXTRA_LAST_TRADE, 0L),
        snapshotEpochMs = intent.getLongExtra(MireiForegroundService.EXTRA_SNAPSHOT_TIME, 0L),
        sourceAgeMs = intent.getLongExtra(MireiForegroundService.EXTRA_SOURCE_AGE, 0L),
        trendScorePercent = intent.getDoubleExtra(MireiForegroundService.EXTRA_TREND, 0.0),
    )
    ...
    private fun card(context: Context, value: String, size: Float): TextView = TextView(context).apply {
        text = value; textSize = size; setTextColor(Color.WHITE); setPadding(8, 10, 8, 10); setBackgroundColor(Color.rgb(24, 34, 43))
    }

    private fun label(context: Context, value: String, bold: Boolean): TextView = TextView(context).apply {
        text = value; textSize = if (bold) 17f else 14f; setTextColor(Color.WHITE)
        if (bold) setTypeface(typeface, android.graphics.Typeface.BOLD); setPadding(4, 10, 4, 5)
    }
    ...
    private fun showStartDialog(activity: Activity) {
        ...
        val outer = LinearLayout(activity).apply { orientation = LinearLayout.VERTICAL; setPadding(16, 4, 16, 14) }
        ...
        val rows = MireiForegroundService.SUPPORTED_MARKETS.mapIndexed { index, market ->
            ...
            val amount = EditText(activity).apply {
                hint = "Modal IDR"; textSize = 15f; setSingleLine(true); inputType = InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_FLAG_DECIMAL
                setText(if (index < 3) "50000" else ""); isEnabled = check.isChecked; setPadding(8, 4, 8, 4)
            }
            ...
            row.addView(check, LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f)); row.addView(amount, LinearLayout.LayoutParams(132, ViewGroup.LayoutParams.WRAP_CONTENT)); list.addView(row)
        ...
        val modeSpinner = Spinner(activity); val modes = arrayOf("AGGRESSIVE", "BALANCED", "SAFETY")
        modeSpinner.adapter = ArrayAdapter(activity, android.R.layout.simple_spinner_dropdown_item, modes); modeSpinner.setSelection(modes.indexOf(prefs.getString("mode", "BALANCED")).coerceAtLeast(0)); outer.addView(modeSpinner, LinearLayout.LayoutParams(-1, ViewGroup.LayoutParams.WRAP_CONTENT))
        ...
        val entryRadio = RadioButton(activity).apply { id = View.generateViewId(); text = "Harga ENTRY COIN — pendekatan harga entry tetap tersedia"; textSize = 15f }
        val capitalRadio = RadioButton(activity).apply { id = View.generateViewId(); text = "MODAL BELI PERTAMA — target/rugi IDR dari modal pertama coin"; textSize = 15f }
        basisGroup.addView(entryRadio); basisGroup.addView(capitalRadio)
        val savedBasis = prefs.getString("risk_basis", RiskReferenceMode.ENTRY_PRICE.name); basisGroup.check(if (savedBasis == RiskReferenceMode.INITIAL_CAPITAL.name) capitalRadio.id else entryRadio.id); outer.addView(basisGroup)
        outer.addView(card(activity, "MODAL BELI PERTAMA tidak menghapus pendekatan harga entry. Ini hanya memilih referensi perhitungan TP/SL. Contoh: modal pertama Rp50.000, TP 1% = target laba Rp500 untuk coin tersebut.", 13f))
        ...
        val slField = EditText(activity).apply { hint = "SL %"; setSingleLine(true); inputType = InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_FLAG_DECIMAL; setText(prefs.getString("manual_sl", "0.50")); setPadding(8, 4, 8, 4) }
        val tpField = EditText(activity).apply { hint = "TP %"; setSingleLine(true); inputType = InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_FLAG_DECIMAL; setText(prefs.getString("manual_tp", "1.00")); setPadding(8, 4, 8, 4) }
        fields.addView(slField, LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f).apply { rightMargin = 10 }); fields.addView(tpField, LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f))
        fields.visibility = if (manualSwitch.isChecked) View.VISIBLE else View.GONE; manualSwitch.setOnCheckedChangeListener { _, checked -> fields.visibility = if (checked) View.VISIBLE else View.GONE }; outer.addView(fields)
        ...
        dialog.setOnShowListener {
            ...
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                ...
                val manual = manualSwitch.isChecked; val sl = slField.text.toString().toDoubleOrNull(); val tp = tpField.text.toString().toDoubleOrNull()
                if (manual && (sl == null || tp == null || sl <= 0.0 || tp <= sl)) { dialog.setTitle("MULAI SESI PAPER · PERIKSA INPUT"); dialog.setMessage("TP manual harus lebih besar dari SL manual dan keduanya harus > 0."); return@setOnClickListener }
    ...
    private fun findButtons(view: View): List<Button> {
        ...
        return buildList { for (index in 0 until view.childCount) addAll(findButtons(view.getChildAt(index))) }
    }
