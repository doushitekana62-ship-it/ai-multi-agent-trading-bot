...
class PaperExecutionEngine(
    ...
    fun seedExistingHolding(exchangeId: String, symbol: String, quoteAmount: Double, marketPrice: Double, stopLossPercent: Double, takeProfitPercent: Double, nowMs: Long, riskReferenceMode: RiskReferenceMode = config.riskReferenceMode, initialCapitalIdr: Double = quoteAmount): ExecutionResult {
        ...
        val targetCosts = if (config.effectiveTakeProfitMode() == TakeProfitMode.MANUAL_NET_IDR) costs.copy(buyFeePercent = 0.0) else costs
    ...
    fun updateRiskTargets(newConfig: TradingConfig) { positions.entries.forEach { (id, position) -> val referenceCapital = if (newConfig.riskReferenceMode == RiskReferenceMode.ENTRY_PRICE) position.stakeIdr else position.riskReferenceCapitalIdr.takeIf { it > 0.0 } ?: position.stakeIdr; val quantity = position.stakeIdr / position.entryPrice; val stopAmount = referenceCapital * newConfig.effectiveStopLossPercent() / 100.0; val stopPrice = if (newConfig.effectiveStopLossPercent() == 0.0) 0.0 else (position.entryPrice - stopAmount / quantity).coerceAtLeast(position.entryPrice * 0.000001); val costs = costsFor(position.symbol); val targets = newConfig.calculateRiskTargets(position.entryPrice, position.stakeIdr, referenceCapital, newConfig.effectiveStopLossPercent(), newConfig.effectiveTakeProfitPercent(), costs); val takeProfitPrice = if (position.takeProfitMode == TakeProfitMode.MANUAL_NET_IDR && position.manualNetProfitTargetIdr != null) { val preserved = newConfig.copy(manualNetProfitTargetIdr = position.manualNetProfitTargetIdr, manualRiskMode = ManualRiskMode.MANUAL); preserved.calculateRiskTargets(position.entryPrice, position.stakeIdr, referenceCapital, newConfig.effectiveStopLossPercent(), newConfig.effectiveTakeProfitPercent(), costs).takeProfitPrice } else targets.takeProfitPrice; positions[id] = position.copy(stopLossPrice = stopPrice, takeProfitPrice = takeProfitPrice, trailingActivationPrice = if (stopPrice == 0.0) 0.0 else position.entryPrice + stopAmount / quantity, riskReferenceMode = newConfig.riskReferenceMode, riskReferenceCapitalIdr = referenceCapital) } }
