...
class PaperExecutionEngine(
    ...
    fun unrealizedNetPnl(positionId: String, marketPrice: Double): Double? { val position = positions[positionId] ?: return null; if (marketPrice <= 0.0) return null; val costs = costsFor(position.symbol); val isInitial = position.entryReason == "initial_holding"; val exitMarketAdjustment = costs.spreadPercent / 2.0 + costs.slippagePercent; val executionPrice = marketPrice * (1.0 - exitMarketAdjustment / 100.0); val entryFee = position.stakeIdr * costs.buyFeePercent / (100.0 + costs.buyFeePercent); val entryNotional = position.stakeIdr - entryFee; val amount = entryNotional / position.entryPrice; val exitNotional = executionPrice * amount; val exitFee = exitNotional * costs.sellFeePercent / 100.0; return (exitNotional - exitFee) - position.stakeIdr }
