...
class PaperExecutionEngine(private val config: TradingConfig = TradingConfig(), private val feePercent: Double = 0.3, private val slippagePercent: Double = 0.05, private val tradeLedger: TradeLedger? = null) {
    ...
    private fun openInternal(exchangeId: String, symbol: String, plan: EntryPlan, nowMs: Long, reservedIdr: Double): ExecutionResult {
        ...
        val executionPrice = plan.entryPrice * (1.0 + slippagePercent / 100.0); val id = "paper-$nowMs-${positions.size + 1}"
    ...
    fun close(positionId: String, marketPrice: Double, reason: String, nowMs: Long = System.currentTimeMillis()): ExecutionResult {
        ...
        val executionPrice = marketPrice * (1.0 - slippagePercent / 100.0); val entryFee = position.stakeIdr * feePercent / (100.0 + feePercent); val entryNotional = position.stakeIdr - entryFee; val amount = entryNotional / position.entryPrice
