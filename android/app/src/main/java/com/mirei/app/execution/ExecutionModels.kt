...
class PaperExecutionEngine(
    ...
    fun updateRiskTargets(newConfig: TradingConfig) { positions.entries.forEach { (id, position) -> val referenceCapital = if (position.riskReferenceMode == RiskReferenceMode.ENTRY_PRICE) position.stakeIdr else position.riskReferenceCapitalIdr.takeIf { it > 0.0 } ?: position.stakeIdr; val quantity = position.stakeIdr / position.entryPrice; val stopPercent = position.positionProfile?.stopLossPercent ?: newConfig.effectiveStopLossPercent(); val stopAmount = referenceCapital * stopPercent / 100.0; val stopPrice = (position.entryPrice - stopAmount / quantity).coerceAtLeast(position.entryPrice * 0.000001); val resolved = position.positionProfile?.let { newConfig.copy(positionProfiles = mapOf(position.symbol to it)).forPosition(position.symbol) } ?: newConfig; val costs = costsFor(position.symbol); val targets = if (position.takeProfitMode == TakeProfitMode.MANUAL_NET_IDR && position.manualNetProfitTargetIdr != null) resolved.copy(manualRiskMode = ManualRiskMode.MANUAL, manualStopLossPercent = stopPercent, manualNetProfitTargetIdr = position.manualNetProfitTargetIdr, manualTakeProfitPercent = null).calculateRiskTargets(position.entryPrice, position.stakeIdr, referenceCapital, stopPercent, resolved.effectiveTakeProfitPercent(), costs) else resolved.calculateRiskTargets(position.entryPrice, position.stakeIdr, referenceCapital, stopPercent, resolved.effectiveTakeProfitPercent(), costs); positions[id] = position.copy(stopLossPrice = stopPrice, takeProfitPrice = targets.takeProfitPrice) } }
