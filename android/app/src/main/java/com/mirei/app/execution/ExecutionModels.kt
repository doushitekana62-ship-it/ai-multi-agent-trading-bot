...
class PaperExecutionEngine(
    ...
    fun seedExistingHolding(exchangeId: String, symbol: String, quoteAmount: Double, marketPrice: Double, stopLossPercent: Double, takeProfitPercent: Double, nowMs: Long, riskReferenceMode: RiskReferenceMode = config.riskReferenceMode, initialCapitalIdr: Double = quoteAmount): ExecutionResult {
        ...
        val before = availableBalanceIdr; val referenceCapital = if (riskReferenceMode == RiskReferenceMode.ENTRY_PRICE) quoteAmount else initialCapitalIdr; val quantity = quoteAmount / marketPrice; val stopAmount = referenceCapital * stopLossPercent / 100.0; val takeAmount = referenceCapital * takeProfitPercent / 100.0
        val stopPrice = if (stopLossPercent == 0.0) 0.0 else (marketPrice - stopAmount / quantity).coerceAtLeast(marketPrice * 0.000001)
        val position = PaperPosition(nextPositionId("paper-initial", nowMs), exchangeId, symbol, quoteAmount, marketPrice, stopPrice, marketPrice + takeAmount / quantity, if (stopPrice == 0.0) 0.0 else marketPrice + stopAmount / quantity, nowMs, "initial_holding", riskReferenceMode, referenceCapital)
    ...
    private fun openInternal(exchangeId: String, symbol: String, plan: EntryPlan, nowMs: Long, reservedIdr: Double, entryReason: String): ExecutionResult {
        ...
        val position = PaperPosition(id, exchangeId, symbol, effectiveStake, executionPrice, stopPrice, executionPrice + takeDistance * entryRatio, if (stopPrice == 0.0) 0.0 else executionPrice + trailingDistance * entryRatio, nowMs, entryReason, plan.riskReferenceMode, plan.riskReferenceCapitalIdr.takeIf { it > 0.0 } ?: effectiveStake)
    ...
    fun fillLimit(orderId: String, marketPrice: Double, nowMs: Long): ExecutionResult {
        ...
        val plan = EntryPlan(true, order.limitPrice, order.limitPrice * 0.995, order.limitPrice * 1.01, order.limitPrice * 1.005, order.quoteAmount, listOf("paper_limit_fill"), RiskReferenceMode.ENTRY_PRICE, order.quoteAmount); val result = openInternal(order.exchangeId, order.symbol, plan, nowMs, order.reservedIdr, "limit_fill"); if (result.success) limitOrders.remove(orderId); return result.copy(orderId = orderId)
    }
    ...
    fun updateRiskTargets(newConfig: TradingConfig) { positions.entries.forEach { (id, position) -> val referenceCapital = if (newConfig.riskReferenceMode == RiskReferenceMode.ENTRY_PRICE) position.stakeIdr else position.riskReferenceCapitalIdr.takeIf { it > 0.0 } ?: position.stakeIdr; val quantity = position.stakeIdr / position.entryPrice; val stopAmount = referenceCapital * newConfig.effectiveStopLossPercent() / 100.0; val stopPrice = if (newConfig.effectiveStopLossPercent() == 0.0) 0.0 else (position.entryPrice - stopAmount / quantity).coerceAtLeast(position.entryPrice * 0.000001); val costs = costsFor(position.symbol); val targets = newConfig.calculateRiskTargets(position.entryPrice, position.stakeIdr, referenceCapital, newConfig.effectiveStopLossPercent(), newConfig.effectiveTakeProfitPercent(), costs); positions[id] = position.copy(stopLossPrice = stopPrice, takeProfitPrice = targets.takeProfitPrice, trailingActivationPrice = if (stopPrice == 0.0) 0.0 else position.entryPrice + stopAmount / quantity, riskReferenceMode = newConfig.riskReferenceMode, riskReferenceCapitalIdr = referenceCapital) } }
