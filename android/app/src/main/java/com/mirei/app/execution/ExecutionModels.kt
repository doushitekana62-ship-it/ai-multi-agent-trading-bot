...
class PaperExecutionEngine(
    ...
    companion object { const val AI_CLOSE_WARMUP_MS = 60_000L }
    private var availableBalanceIdr = config.totalCapitalIdr; private val positions = linkedMapOf<String, PaperPosition>(); private val limitOrders = linkedMapOf<String, PaperLimitOrder>(); private var positionSequence = 0L
    ...
    private fun openInternal(exchangeId: String, symbol: String, plan: EntryPlan, nowMs: Long, reservedIdr: Double, entryReason: String): ExecutionResult {
        ...
        val id = nextPositionId("paper", nowMs); val stopPrice = if (plan.stopLossPrice == 0.0) 0.0 else (executionPrice - stopDistance * entryRatio).coerceAtLeast(executionPrice * 0.000001)
        val takePrice = if (plan.takeProfitMode == TakeProfitMode.MANUAL_NET_IDR) plan.takeProfitPrice else executionPrice + takeDistance * entryRatio
        val position = PaperPosition(id, exchangeId, symbol, effectiveStake, executionPrice, stopPrice, takePrice, if (stopPrice == 0.0) 0.0 else executionPrice + trailingDistance * entryRatio, nowMs, entryReason, plan.riskReferenceMode, plan.riskReferenceCapitalIdr.takeIf { it > 0.0 } ?: effectiveStake, plan.takeProfitMode, plan.manualNetProfitTargetIdr)
    ...
    fun close(positionId: String, marketPrice: Double, reason: String, nowMs: Long = System.currentTimeMillis()): ExecutionResult {
        ...
        val executedAt = nowMs + costs.executionLatencyMs; tradeLedger?.recordClosed(position, executionPrice, entryFee + exitFee, netPnl, executedAt, reason); availableBalanceIdr += proceedsAfterFee; positions.remove(positionId)
    ...
    fun unrealizedNetPnl(positionId: String, marketPrice: Double): Double? {
        val position = positions[positionId] ?: return null
        if (marketPrice <= 0.0) return null
        val costs = costsFor(position.symbol); val isInitial = position.entryReason == "initial_holding"; val exitMarketAdjustment = costs.spreadPercent / 2.0 + costs.slippagePercent; val executionPrice = marketPrice * (1.0 - exitMarketAdjustment / 100.0)
        val entryFee = if (isInitial) 0.0 else position.stakeIdr * costs.buyFeePercent / (100.0 + costs.buyFeePercent); val entryNotional = position.stakeIdr - entryFee; val amount = entryNotional / position.entryPrice; val exitNotional = executionPrice * amount; val exitFee = exitNotional * costs.sellFeePercent / 100.0
        return (exitNotional - exitFee) - position.stakeIdr
    }

    fun placeLimit(exchangeId: String, symbol: String, quoteAmount: Double, limitPrice: Double, nowMs: Long): ExecutionResult {
        if (quoteAmount <= 0.0) return ExecutionResult(false, remainingBalanceIdr = availableBalanceIdr, error = "invalid_quote_amount"); if (limitPrice <= 0.0) return ExecutionResult(false, remainingBalanceIdr = availableBalanceIdr, error = "invalid_limit_price"); if (positions.size + limitOrders.size >= config.maxOpenPositions) return ExecutionResult(false, remainingBalanceIdr = availableBalanceIdr, error = "paper_position_limit"); if (quoteAmount > availableBalanceIdr) return ExecutionResult(false, remainingBalanceIdr = availableBalanceIdr, error = "insufficient_paper_balance")
        val before = availableBalanceIdr; val id = "limit-$nowMs-${limitOrders.size + 1}"; availableBalanceIdr -= quoteAmount; limitOrders[id] = PaperLimitOrder(id, exchangeId, symbol, quoteAmount, limitPrice, quoteAmount, nowMs); return ExecutionResult(true, orderId = id, remainingBalanceIdr = availableBalanceIdr, reason = "limit_order_accepted", balanceBeforeIdr = before)
    }
    fun fillLimit(orderId: String, marketPrice: Double, nowMs: Long): ExecutionResult {
        val order = limitOrders[orderId] ?: return ExecutionResult(false, remainingBalanceIdr = availableBalanceIdr, error = "paper_limit_order_not_found"); if (marketPrice <= 0.0) return ExecutionResult(false, remainingBalanceIdr = availableBalanceIdr, error = "invalid_market_price"); if (marketPrice > order.limitPrice) return ExecutionResult(false, remainingBalanceIdr = availableBalanceIdr, error = "paper_limit_not_reached")
        val plan = EntryPlan(true, order.limitPrice, order.limitPrice * 0.995, order.limitPrice * 1.01, order.limitPrice * 1.005, order.quoteAmount, listOf("paper_limit_fill")); val result = openInternal(order.exchangeId, order.symbol, plan, nowMs, order.reservedIdr, "limit_fill"); if (result.success) limitOrders.remove(orderId); return result.copy(orderId = orderId)
    }
    ...
    fun updateRiskTargets(newConfig: TradingConfig) { positions.entries.forEach { (id, position) -> val referenceCapital = if (newConfig.riskReferenceMode == RiskReferenceMode.ENTRY_PRICE) position.stakeIdr else position.riskReferenceCapitalIdr.takeIf { it > 0.0 } ?: position.stakeIdr; val quantity = position.stakeIdr / position.entryPrice; val stopAmount = referenceCapital * newConfig.effectiveStopLossPercent() / 100.0; val stopPrice = if (newConfig.effectiveStopLossPercent() == 0.0) 0.0 else (position.entryPrice - stopAmount / quantity).coerceAtLeast(position.entryPrice * 0.000001); val costs = costsFor(position.symbol); val targets = newConfig.calculateRiskTargets(position.entryPrice, position.stakeIdr, referenceCapital, newConfig.effectiveStopLossPercent(), newConfig.effectiveTakeProfitPercent(), costs); val tpMode = position.takeProfitMode; val takeProfitPrice = if (tpMode == TakeProfitMode.MANUAL_NET_IDR && position.manualNetProfitTargetIdr != null) { val preserved = newConfig.copy(manualNetProfitTargetIdr = position.manualNetProfitTargetIdr, manualRiskMode = com.mirei.app.core.ManualRiskMode.MANUAL); preserved.calculateRiskTargets(position.entryPrice, position.stakeIdr, referenceCapital, newConfig.effectiveStopLossPercent(), newConfig.effectiveTakeProfitPercent(), costs).takeProfitPrice } else targets.takeProfitPrice; positions[id] = position.copy(stopLossPrice = stopPrice, takeProfitPrice = takeProfitPrice, trailingActivationPrice = if (stopPrice == 0.0) 0.0 else position.entryPrice + stopAmount / quantity, riskReferenceMode = newConfig.riskReferenceMode, riskReferenceCapitalIdr = referenceCapital) } }
    ...
    fun restoreState(state: PaperEngineState) { require(state.availableBalanceIdr >= 0.0) { "invalid_paper_balance_state" }; require(state.positions.size <= config.maxOpenPositions) { "paper_position_limit" }; positions.clear(); limitOrders.clear(); state.positions.forEach { position -> require(position.stakeIdr > 0.0 && position.entryPrice > 0.0); positions[position.id] = position }; availableBalanceIdr = state.availableBalanceIdr; positionSequence = state.positions.mapNotNull { it.id.substringAfterLast('-').toLongOrNull() }.maxOrNull() ?: 0L }
