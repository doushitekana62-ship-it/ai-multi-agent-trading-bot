...
class MireiForegroundService : Service() {
    ...
    private fun runScanner() {
        ...
        lastScannerSummary = ranked.joinToString("\n") { (pair, snapshot) ->
            ...
            "$pair | trend ${"%.2f".format(snapshot.trendScorePercent)} | mom ${"%.3f".format(snapshot.momentumPercent)}% | volume share ${"%.1f".format(share)}%"
        }
    ...
    private fun publishStatus(status: PaperRuntimeStatus) {
        val intent = Intent(ACTION_STATUS).setPackage(packageName).apply {
            ...
            putExtra(EXTRA_PRICE, status.marketPrice); putExtra(EXTRA_BID, status.marketBidPrice); putExtra(EXTRA_ASK, status.marketAskPrice); putExtra(EXTRA_HIGH_24H, status.marketHigh24h); putExtra(EXTRA_LOW_24H, status.marketLow24h); putExtra(EXTRA_VOLUME_24H, status.marketVolume24h)
            putExtra(EXTRA_EQUITY, status.equityIdr); putExtra(EXTRA_BALANCE, status.availableBalanceIdr); putExtra(EXTRA_PNL, status.dailyPnlIdr); putExtra(EXTRA_POSITIONS, status.activePositions.size); putExtra(EXTRA_CONFIDENCE, status.lastDecision?.confidence ?: 0.0); putExtra(EXTRA_ACTION, status.lastDecision?.action?.name ?: "HOLD"); putExtra(EXTRA_RATIONALE, status.lastDecision?.rationale ?: "no_decision")
            ...
            putExtra(EXTRA_ENTRY_REASONS, status.entryPlanReasons.joinToString(" | ")); putExtra(EXTRA_MOMENTUM, status.marketMomentumPercent); putExtra(EXTRA_VOLATILITY, status.marketVolatilityPercent); putExtra(EXTRA_SENTIMENT, status.marketSentimentScore); putExtra(EXTRA_FORECAST_CONFIDENCE, status.forecastConfidence); putExtra(EXTRA_SPREAD, status.marketSpreadPercent)
            putExtra(EXTRA_CHANGE_TICK, status.changeSinceLastTickPercent); putExtra(EXTRA_CHANGE_1M, status.change1mPercent); putExtra(EXTRA_CHANGE_5M, status.change5mPercent); putExtra(EXTRA_CHANGE_15M, status.change15mPercent); putExtra(EXTRA_FLOW, status.tradeFlowPercent); putExtra(EXTRA_TREND, status.trendScorePercent); putExtra(EXTRA_TRADE_COUNT, status.tradeCount); putExtra(EXTRA_BUY_VOLUME, status.buyVolume); putExtra(EXTRA_SELL_VOLUME, status.sellVolume)
            putExtra(EXTRA_LAST_TRADE, status.lastTradeEpochMs); putExtra(EXTRA_SNAPSHOT_TIME, status.snapshotEpochMs); putExtra(EXTRA_SOURCE_AGE, status.sourceAgeMs); putExtra(EXTRA_MARKET_FRESH, status.marketDataFresh); putExtra(EXTRA_INTERNET, status.internetAvailable); putExtra(EXTRA_EXCHANGE_HEALTHY, status.exchangeHealthy); putExtra(EXTRA_ERROR, status.lastError); putExtra(EXTRA_POSITIONS_DETAIL, status.activePositions.joinToString("\n") { p -> "${p.symbol} | Rp ${"%.0f".format(p.stakeIdr)} | entry ${"%.2f".format(p.entryPrice)} | TP ${"%.2f".format(p.takeProfitPrice)} | SL ${"%.2f".format(p.stopLossPrice)}" }); putExtra(EXTRA_SCANNER, status.scannerSummary); putExtra(EXTRA_TICK, status.lastTickEpochMs)
        }
    ...
    companion object {
        ...
        val SUPPORTED_MARKETS = listOf("BTC/IDR", "ETH/IDR", "HYPE/IDR", "FARTCOIN/IDR", "SOL/IDR", "XRP/IDR", "ETH/IDR", "DOGE/IDR", "ADA/IDR", "SUI/IDR").distinct()
