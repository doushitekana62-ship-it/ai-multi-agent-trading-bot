...
class MireiForegroundService : Service() {
        ...
        try {
            ...
            when (intent?.action) {
                ...
                ACTION_REFRESH -> worker.post {
                    val status = if (sessionStarted) {
                        runtime.tick(System.currentTimeMillis(), RuntimeEnvironment(internetAvailable, exchangeId == DEFAULT_EXCHANGE)).also { persistDecisions(it) }
                    } else {
                        runScanner()
    ...
    private fun persistDecisions(status: PaperRuntimeStatus) {
        ...
        status.recentExecutions.forEach { execution -> db.recordAudit("EXECUTION", "${execution.reason ?: "execution"}|${execution.orderId ?: "-"}|pnl=${execution.pnlIdr}", status.lastTickEpochMs) }
    }
    ...
    private fun handleRuntimeFailure(message: String, error: Throwable) {
        ...
        runCatching { publish("ERROR · $message · ${error.javaClass.simpleName}") }
    ...
    private fun humanError(error: String): String = error.replace('_', ' ')
