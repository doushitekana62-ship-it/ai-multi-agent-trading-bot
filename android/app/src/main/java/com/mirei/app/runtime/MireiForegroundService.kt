...
class MireiForegroundService : Service() {
        ...
        try {
            ...
            when (intent?.action) {
                ...
                ACTION_REFRESH -> worker.post {
                    if (controller.state == MireiState.RUNNING) {
                        val status = runtime.tick(System.currentTimeMillis(), RuntimeEnvironment(internetAvailable, exchangeId == DEFAULT_EXCHANGE))
                        publishStatus(status); persistDecisions(status)
    ...
    private fun startRuntime(intent: Intent) {
        ...
        worker.post {
            ...
            if (seeded.size != allocations.size || seeded.any { !it.success }) {
                publishStatus(runtime.status(RuntimeEnvironment(internetAvailable, false))); return@post
            }
    ...
    private fun parseAllocations(raw: String): LinkedHashMap<String, Double> = linkedMapOf<String, Double>().apply {
        raw.split(';').forEach { token -> val parts = token.split('='); val amount = parts.getOrNull(1)?.toDoubleOrNull(); val pair = parts.getOrNull(0)?.trim(); if (pair != null && pair in SUPPORTED_MARKETS && amount != null && amount > 0.0) put(pair, amount) }
    }
    ...
    private fun closeAll() {
        val wasRunning = controller.state == MireiState.RUNNING; controller.closeAll(); worker.post { val status = runtime.closeAll(System.currentTimeMillis(), RuntimeEnvironment(internetAvailable, exchangeId == DEFAULT_EXCHANGE)); if (wasRunning) controller.start(); publishStatus(status); persistDecisions(status); if (wasRunning) worker.post(runtimeLoop) }
    }
    ...
    private fun runScanner() {
        ...
        lastScannerSummary = ranked.joinToString("\n") { (pair, snapshot) -> val share = if (totalVolume > 0.0) snapshot.volume24h / totalVolume * 100.0 else 0.0; "$pair | trend ${"%.2f".format(snapshot.trendScorePercent)} | mom ${"%.3f".format(snapshot.momentumPercent)}% | volume share ${"%.1f".format(share)}%" }
    ...
    override fun onDestroy() {
        ...
        workerThread.takeIf { ::workerThread.isInitialized }?.quitSafely(); networkCallback = null; connectivityManager = null; super.onDestroy()
    }

    private fun handleRuntimeFailure(message: String, error: Throwable) { controller.onEngineError(); runCatching { publish("$message — Mirei ERROR: ${error.javaClass.simpleName}") }; runCatching { publishHealth() }; stopSelf() }
    ...
    private fun loadConfig(): TradingConfig { val mode = runCatching { ScalpingMode.valueOf(prefs.getString(KEY_MODE, ScalpingMode.BALANCED.name)!!) }.getOrDefault(ScalpingMode.BALANCED); val manual = prefs.getBoolean(KEY_MANUAL, false); val sl = prefs.getString(KEY_MANUAL_SL, null)?.toDoubleOrNull(); val tp = prefs.getString(KEY_MANUAL_TP, null)?.toDoubleOrNull(); return if (manual && sl != null && tp != null && tp > sl) TradingConfig(mode = mode, decisionMode = DecisionMode.SUGGESTION, manualRiskMode = com.mirei.app.core.ManualRiskMode.MANUAL, manualStopLossPercent = sl, manualTakeProfitPercent = tp) else TradingConfig(mode = mode, decisionMode = DecisionMode.SUGGESTION) }
    ...
    companion object {
        ...
        val SUPPORTED_MARKETS = listOf("BTC/IDR", "ETH/IDR", "HYPE/IDR", "FARTCOIN/IDR", "SOL/IDR", "XRP/IDR", "ETH/IDR", "DOGE/IDR", "ADA/IDR", "SUI/IDR", "TRX/IDR").distinct()
