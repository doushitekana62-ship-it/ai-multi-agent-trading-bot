...
class MireiForegroundService : Service() {
        ...
        try {
            ...
            when (intent?.action) {
                ...
                ACTION_STOP -> {
                    ...
                    // Deliberately keep the foreground service and runtime state alive.
                    // STOP pauses execution; it does not reset the paper portfolio.
                    publishHealth()
                }
    ...
    private fun persistDecisions(status: PaperRuntimeStatus) {
        ...
        status.recentExecutions.forEach { execution ->
            db.recordAudit(
                "EXECUTION",
                "${execution.reason ?: "execution"}|${execution.orderId ?: "-"}|pnl=${execution.pnlIdr}",
    ...
    private fun runScanner() {
        ...
        lastScannerSummary = ranked.joinToString("\n") { (pair, snapshot) ->
            ...
            listOf(
                pair,
                "%.2f".format(Locale.US, snapshot.price),
                "%.3f".format(Locale.US, snapshot.change1mPercent),
                "%.3f".format(Locale.US, snapshot.momentumPercent),
                "%.3f".format(Locale.US, snapshot.trendScorePercent),
                "%.2f".format(Locale.US, share),
            ).joinToString("|")
        }
    ...
    private fun publishStatus(status: PaperRuntimeStatus) {
        val intent = Intent(ACTION_STATUS).setPackage(packageName).apply {
            ...
            putExtra(EXTRA_AGENT_SUMMARY, status.decisionsBySymbol.entries.joinToString("\n\n") { (pair, decision) ->
                "$pair => " + decision.observations.joinToString(" | ") { observation ->
                    "${observation.agent.name}:${observation.action.name} ${(observation.confidence * 100).toInt()}% ${observation.rationale}"
                }
            })
            ...
            putExtra(EXTRA_RECENT_EXECUTIONS, status.recentExecutions.joinToString("\n") { execution ->
                "${execution.reason ?: "execution"} | PnL Rp ${"%.2f".format(Locale.US, execution.pnlIdr)} | ${execution.orderId ?: "-"}"
            })
        ...
        val event = when {
            ...
            status.recentExecutions.isNotEmpty() -> status.recentExecutions.last().let { execution ->
                "${execution.reason ?: "execution"} · PnL Rp ${"%.2f".format(Locale.US, execution.pnlIdr)}"
            }
