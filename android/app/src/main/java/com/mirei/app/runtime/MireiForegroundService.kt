...
class MireiForegroundService : Service() {
    ...
    private fun runScanner() {
        ...
        lastScannerSummary = ranked.joinToString("\n") { (pair, snapshot) ->
            ...
            listOf(pair, "%.2f".format(java.util.Locale.US, snapshot.price), "%.3f".format(java.util.Locale.US, snapshot.change1mPercent), "%.3f".format(java.util.Locale.US, snapshot.momentumPercent), "%.3f".format(java.util.Locale.US, snapshot.trendScorePercent), "%.2f".format(java.util.Locale.US, share)).joinToString("|")
        }
    ...
    override fun onDestroy() {
        ...
        networkCallback = null; connectivityManager = null; super.onDestroy()
    }
    ...
    private fun publishStatus(status: PaperRuntimeStatus) {
        val intent = Intent(ACTION_STATUS).setPackage(packageName).apply {
            ...
            putExtra(EXTRA_POSITIONS_DETAIL, status.activePositions.joinToString("\n") { p ->
                ...
                "${p.symbol}|stake=${"%.2f".format(java.util.Locale.US, p.stakeIdr)}|entry=${"%.2f".format(java.util.Locale.US, p.entryPrice)}|current=${"%.2f".format(java.util.Locale.US, current)}|value=${"%.2f".format(java.util.Locale.US, value)}|unrealized=${"%.2f".format(java.util.Locale.US, unrealized)}|tp=${"%.2f".format(java.util.Locale.US, p.takeProfitPrice)}|sl=${"%.2f".format(java.util.Locale.US, p.stopLossPrice)}|tp_pct=${"%.3f".format(java.util.Locale.US, tpDistance)}|sl_pct=${"%.3f".format(java.util.Locale.US, slDistance)}"
            })
        ...
        publish("Mirei ${controller.state.name} · $symbol · $action · ${status.activePositions.size} position(s)")
    }
