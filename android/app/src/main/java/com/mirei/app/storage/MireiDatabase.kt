...
class MireiDatabase(context: Context) : SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION) {
    override fun onCreate(db: SQLiteDatabase) {
        db.execSQL("""CREATE TABLE trades (id TEXT PRIMARY KEY, exchange_id TEXT NOT NULL, symbol TEXT NOT NULL, side TEXT NOT NULL, status TEXT NOT NULL, entry_price REAL, exit_price REAL, stake_idr REAL NOT NULL, fee_idr REAL NOT NULL DEFAULT 0, pnl_idr REAL NOT NULL DEFAULT 0, opened_at INTEGER NOT NULL, closed_at INTEGER, exit_reason TEXT)""")
    ...
    fun recordTradeClosed(positionId: String, exitPrice: Double, feeIdr: Double, pnlIdr: Double, closedAtEpochMs: Long, exitReason: String) {
        ...
        val updated = writableDatabase.update("trades", ContentValues().apply { put("status", "CLOSED"); put("exit_price", exitPrice); put("fee_idr", feeIdr); put("pnl_idr", pnlIdr); put("closed_at", closedAtEpochMs); put("exit_reason", exitReason) }, "id = ?", arrayOf(positionId))
...
data class TradeRow(val id: String, val exchangeId: String, val symbol: String, val side: String, val status: String, val entryPrice: Double?, val exitPrice: Double?, val stakeIdr: Double, val feeIdr: Double, val pnlIdr: Double, val openedAtEpochMs: Long, val closedAtEpochMs: Long?, val exitReason: String?)
