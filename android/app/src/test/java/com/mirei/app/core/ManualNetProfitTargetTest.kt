...
class ManualNetProfitTargetTest {
    ...
    fun manualTargetIsNetAfterInstrumentCosts() {
        ...
        val engine = PaperExecutionEngine(
            config = config,
            tradeLedger = null,
            useInstrumentCosts = true,
        )
        val opened = engine.open(
            "indodax", "BTC/IDR",
            EntryPlan(true, 1_000_000.0, targets.stopLossPrice, targets.takeProfitPrice, 1_005_000.0, 50_000.0, listOf("test")),
            1_000L,
        )
        ...
        assertTrue("net pnl should meet Rp30 target, got ${closed.pnlIdr}", closed.pnlIdr >= 30.0)
    }
}
