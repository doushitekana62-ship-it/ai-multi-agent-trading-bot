...
class PaperExecutionEngineTest {
    ...
    @Test fun ledgerFailureDoesNotCommitCloseState() { val ledger = object : TradeLedger { override fun recordOpened(position: PaperPosition, entryFeeIdr: Double) = Unit; override fun recordClosed(position: PaperPosition, exitPrice: Double, feeIdr: Double, pnlIdr: Double, closedAtEpochMs: Long, exitReason: String) { error("ledger_down") } }; val engine = PaperExecutionEngine(tradeLedger = ledger); val opened = engine.open("paper", "BTC/IDR", plan, 1000L); val balanceBefore = engine.availableBalanceIdr(); var failed = false; try { engine.close(opened.orderId!!, 1_010_000.0, "take_profit", 2000L) } catch (_: IllegalStateException) { failed = true }; assertTrue(failed); assertEquals(1, engine.positionCount()); assertEquals(balanceBefore, engine.availableBalanceIdr(), 0.001) }

    @Test fun limitOrdersReserveCapitalAndCancellationReleasesIt() { val config = TradingConfig(totalCapitalIdr = 150_000.0, positionSizeIdr = 50_000.0, maxOpenPositions = 3); val engine = PaperExecutionEngine(config = config); val first = engine.placeLimit("paper", "BTC/IDR", 50_000.0, 1_000_000.0, 1000L); val second = engine.placeLimit("paper", "ETH/IDR", 50_000.0, 2_000_000.0, 1001L); val third = engine.placeLimit("paper", "SOL/IDR", 50_000.0, 3_000_000.0, 1002L); val fourth = engine.placeLimit("paper", "XRP/IDR", 50_000.0, 4_000_000.0, 1003L); assertTrue(first.success); assertTrue(second.success); assertTrue(third.success); assertTrue(!fourth.success); assertEquals(3, engine.pendingLimitOrders().size); assertEquals(0.0, engine.availableBalanceIdr(), 0.001); assertTrue(engine.cancelLimit(second.orderId!!)); assertEquals(2, engine.pendingLimitOrders().size); assertEquals(50_000.0, engine.availableBalanceIdr(), 0.001) }
