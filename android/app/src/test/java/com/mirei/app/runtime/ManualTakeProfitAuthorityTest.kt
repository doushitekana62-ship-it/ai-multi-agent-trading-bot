...
class ManualTakeProfitAuthorityTest {
    @Test fun manualNetTargetIsTheOnlyTpSourceForThePosition() {
        ...
        val config = TradingConfig(
            ...
            manualNetProfitTargetIdr = 113.0,
        ...
        assertEquals(1, hit.activePositions.size)
        assertEquals(3_000L, hit.activePositions.single().openedAtEpochMs)
    }
}

private class ManualTpMarket(var price: Double) : PaperMarketDataSource {
    override fun snapshot(symbol: String) = MarketSnapshotFactory.bullish(symbol, price)
}

private object MarketSnapshotFactory {
    fun bullish(symbol: String, price: Double) = com.mirei.app.core.MarketSnapshot(
        symbol = symbol,
        price = price,
        momentumPercent = 1.0,
