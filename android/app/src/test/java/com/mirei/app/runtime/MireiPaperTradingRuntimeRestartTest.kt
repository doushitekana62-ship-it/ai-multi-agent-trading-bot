...
class MireiPaperTradingRuntimeRestartTest {
    @Test
    fun restoredPortfolioSurvivesStopAndStartAndCanAddValidBuy() {
        ...
        assertTrue("STOP/START must preserve the persisted position", afterRestart.activePositions.any { it.id == started.activePositions.single().id })
        assertTrue("A valid BUY after STOP/START may fill an additional slot", afterRestart.activePositions.size >= 1)
    }
