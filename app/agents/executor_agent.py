...
class ExecutorAgent:
    ...
    def __init__(self, mode: str):
        if mode == "paper":
            self.gateway: TradingGateway = PaperGateway()
        elif mode == "live":
    ...
    def _account(self, user_id):
        account = execute_one(
            self.db.table("trading_accounts").select("*").eq("user_id", user_id).eq("mode", self.mode)
        )
        if not account:
            raise RuntimeError("Trading account is not initialized")
        return account
    ...
    def open_position(self, user_id, signal):
        ...
        # Fail before creating an exchange/paper order when the paper account cannot fund it.
        ...
        if not order_rows:
            raise RuntimeError("Could not create internal order record")
        order_row = order_rows[0] if isinstance(order_rows, list) else order_rows

        try:
            order = self.gateway.buy(signal.symbol, signal.entry_price, signal.quantity)
            total_cost = order.price * order.quantity + order.fee
            if self.mode == "paper":
                if float(account.get("cash_balance") or 0) < total_cost:
                    raise RuntimeError("Insufficient paper balance after slippage")
                execute_data(
                    self.db.table("trading_accounts")
                    .update({"cash_balance": float(account["cash_balance"]) - total_cost})
                    .eq("id", account["id"]),
                    [],
                )
            position_rows = execute_data(
            ...
            if not position_rows:
                raise RuntimeError("Exchange order filled but position record was not created")
            ...
            execute_data(
                self.db.table("trade_logs").insert(
                    {
                        "user_id": user_id,
                        "position_id": position["id"],
                        "mode": self.mode,
            ...
            try:
                execute_data(
                    self.db.table("orders").update({"status": "rejected"}).eq("id", order_row["id"]),
                    [],
                )
            except Exception:
                pass
    ...
    def close_position(self, user_id, position, price, reason):
        ...
        result = self.db.table("positions").update(
            {
                "status": status,
        ...
        ).eq("id", position["id"]).eq("user_id", user_id).eq("mode", self.mode).eq("status", "open").execute()
        result_data = getattr(result, "data", None) or []
        if not result_data:
            raise RuntimeError("Position was already closed")
        if self.mode == "paper":
            account = self._account(user_id)
            proceeds = order.price * order.quantity - order.fee
            execute_data(
                self.db.table("trading_accounts")
                .update({"cash_balance": float(account["cash_balance"]) + proceeds})
                .eq("id", account["id"]),
                [],
            )
        execute_data(
            self.db.table("trade_logs").insert(
                {
                    ...
                    "mode": self.mode,
                    "action": reason,
                    "detail": {"exit_price": order.price, "pnl": pnl, "fee": order.fee},
                }
            ),
            [],
        )
