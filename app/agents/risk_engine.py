...
class RiskEngine:
    """Single risk controller for every strategy."""

    def __init__(self, daily_loss_limit_percent=3.0, max_drawdown_percent=5.0,
                 max_total_open_risk_percent=1.5, max_loss_streak=3, cooldown_minutes=30):
    ...
    def evaluate(self, daily_pnl: float, daily_start_balance: float, equity: float,
                 peak_equity: float, open_risk_percent: float, loss_streak: int,
                 cooldown_until: str | None = None, pair_quarantined: bool = False,
                 spread_percent: float = 0.0) -> RiskDecision:
        reasons: list[str] = []
