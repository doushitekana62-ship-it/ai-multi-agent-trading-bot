...
class TradingScheduler:
    ...
    def _ensure_account(self, db, user_id, mode):
        ...
        return db.table("trading_accounts").insert({"user_id": user_id, "mode": mode, "initial_balance": initial,
            "cash_balance": initial, "daily_start_balance": initial}).execute().data[0]
    ...
    def _loss_streak(self, db, user_id, mode):
        rows = db.table("positions").select("pnl,status,closed_at").eq("user_id", user_id).eq("mode", mode).neq("status", "open").order("closed_at", desc=True).limit(10).execute().data or []
        streak = 0
        for row in rows:
    ...
    def _cooldown_until(self, db, user_id, mode):
        row = db.table("risk_events").select("created_at").eq("user_id", user_id).eq("mode", mode).eq("event_type", "loss_streak").order("created_at", desc=True).limit(1).execute().data
        if not row:
            return None
        created = datetime.fromisoformat(str(row[0]["created_at"]).replace("Z", "+00:00"))
    ...
    def run_cycle(self):
        ...
        for user in users:
            if not user.get("bot_enabled", False):
                continue
            user_id = str(user["id"])
            ...
            peak_equity = max(float(account.get("initial_balance") or equity), equity)
            ...
            if loss_streak >= settings.max_loss_streak:
                db.table("risk_events").insert({"user_id": user_id, "mode": mode, "event_type": "loss_streak",
                    "severity": "warning", "detail": {"loss_streak": loss_streak}}).execute()
            ...
            for alloc in allocations:
                ...
                try:
                    ...
                    if open_pos:
                        if price >= float(open_pos["tp_price"]):
                            ExecutorAgent(mode).close_position(user_id, open_pos, price, "tp_hit")
                        elif price <= float(open_pos["sl_price"]):
                            ExecutorAgent(mode).close_position(user_id, open_pos, price, "sl_hit")
                        continue
                    if decision.action != "OPEN" or not risk_base.allowed or self._pair_quarantined(db, user_id, mode, symbol):
                        continue
                    ...
                    if signal:
                        adjusted = signal.__class__(signal.symbol, signal.entry_price, signal.tp_price, signal.sl_price,
                            signal.quantity * risk_base.risk_multiplier, signal.allocation_percent, signal.compound_multiplier)
