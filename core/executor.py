...
The executor never decides direction. It only executes an already-approved
candidate and enforces basic portfolio invariants.
"""
...
@dataclass
class Order:
    order_id:str; symbol:str; side:str; quantity:float; price:float; status:str; created_at:datetime
    filled_at:Optional[datetime]; filled_quantity:float; filled_price:float; stop_loss:Optional[float]; take_profit:Optional[float]; metadata:Dict[str,Any]

class Executor:
    def __init__(self, config:Optional[Dict[str,Any]]=None):
        self.config=config or {}; self.exchange_mode=str(self.config.get("exchange_mode","paper")).lower()
        self.paper_trading=PaperTrading(self.config.get("paper",{})); self.alpaca_bridge=AlpacaBridge(self.config.get("alpaca",{})); self.indodax_bridge=None
        self.active_orders={}; self.order_history=[]; self.active_positions={}
        self.max_open_positions=min(3,max(1,int(self.config.get("max_open_positions",3))))
        self.daily_loss_limit=float(self.config.get("daily_loss_limit",0.05)); self.daily_starting_equity=0.0; self.daily_realized_pnl=0.0; self.daily_pnl=0.0; self.daily_trades=0; self.order_id_counter=0
    ...
    def configure_exchange(self,exchange_mode:str,credentials:Optional[Dict[str,Any]]=None)->None:
        ...
        if self.exchange_mode=="indodax":
            self.indodax_bridge=IndodaxBridge({"api_key":credentials.get("api_key"),"secret":credentials.get("api_secret") or credentials.get("secret"),"enable_trading":credentials.get("enable_trading",False)})
    ...
    def execute(self,symbol:str,action:str,confidence:float,position_size:float,stop_loss:Optional[float]=None,take_profit:Optional[float]=None,market_price:Optional[float]=None)->Optional[Order]:
        action=action.upper()
        if action not in {"BUY","SELL","STRONG_BUY","STRONG_SELL"} or not 0<position_size<=1:return None
        ...
        if side=="BUY" and symbol in self.active_positions:return None
        if side=="SELL" and symbol not in self.active_positions:return None
        if side=="BUY" and len(self.active_positions)>=self.max_open_positions:return None
    ...
    def _create_order(self,symbol,side,quantity,price,stop_loss,take_profit):
        ...
        return Order(f"ORD_{self.order_id_counter:06d}",symbol,side,quantity,price,OrderStatus.PENDING.value,datetime.now(timezone.utc),None,0.0,0.0,stop_loss,take_profit,{})

    def _execute_order(self,order):
        try:
            if self.exchange_mode=="paper":
                ok=self.paper_trading.execute_order(order.symbol,order.side,order.quantity,order.price); result={"paper":True} if ok else None
            elif self.exchange_mode=="indodax":
                if not self.indodax_bridge: raise RuntimeError("Indodax is not configured")
            ...
            else: raise ValueError(f"Unsupported exchange mode: {self.exchange_mode}")
    ...
    def _get_execution_realized_pnl(self,order):
        if self.exchange_mode=="paper" and self.paper_trading.trade_history:
            ...
            if trade.get("symbol")==order.symbol:return float(trade.get("pnl",0.0))
        position=self.active_positions.get(order.symbol)
        return (order.filled_price-position["entry_price"])*order.filled_quantity if position else 0.0
    ...
    def monitor_positions(self,market_prices:Optional[Dict[str,float]]=None):
    ...
    def _close_position(self,symbol,price,reason):
        ...
        order=self._create_order(symbol,"SELL",position["quantity"],price,None,None);order.metadata["close_reason"]=reason
