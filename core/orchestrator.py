...
class Orchestrator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.use_unified_data = self.config.get("use_unified_data", True)

        # ============================================================
        # MIMIC TRADER - Referensi trader profesional
        # ============================================================

        self.mimic_trader = MimicTrader(self.config.get("mimic_trader", {}))
        self.use_mimic_trader = self.config.get("use_mimic_trader", True)
        self.mimic_weight = self.config.get("mimic_weight", 0.25)  # Bobot MimicTrader dalam voting
        ...
        self.min_confidence_threshold = float(self.config.get("min_confidence", 0.40))
        self.high_confidence_threshold = float(self.config.get("high_confidence", 0.65))

        # ============================================================
        # CONFLICT HANDLING
        # ============================================================

        self.conflict_penalty = float(self.config.get("conflict_penalty", 0.08))
        self.minor_conflict_penalty = float(self.config.get("minor_conflict_penalty", 0.03))
        self.severe_conflict_penalty = float(self.config.get("severe_conflict_penalty", 0.15))
        ...
        self.default_position_size = float(self.config.get("default_position_size", 0.05))
        self.min_position_size = float(self.config.get("min_position_size", 0.01))
