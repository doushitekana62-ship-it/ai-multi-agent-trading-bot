...
async def _fresh_public_indodax(path):
    try:
        sep="&" if "?" in path else "?"
        url=f"https://indodax.com/api{path}{sep}_live={int(time.time()*1000)}"
...
def _clean_pair(value):
    ...
    p=(value or "btc_idr").strip().lower().replace("/","_")
    return p if p in allowed else "btc_idr"
...
async def _supabase_observations(env,pair):
    ...
    try:
        ...
        url=f"{base}/rest/v1/market_observations?select=observed_at,minute_bucket,price,source,observation_type,trade_count,move_from_previous_pct,pulse_status,raw_observation&symbol=eq.{symbol}&order=observed_at.desc&limit=360"
