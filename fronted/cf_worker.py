...
def _verify_token(scope, token: str):
    try:
        ...
        if json.loads(_b64url_decode(header)).get("alg")!="HS256": return None
...
async def _market_insights(scope):
    ...
    for pair,ticker in raw.items():
        ...
        try:
            ...
            if last<=0: continue
            width=high-low; pos=((last-low)/width*100.0) if width>0 else 50.0
    ...
    return {"items":items[:20],"total_idr_pairs":len(items),"scalping_pairs":[item["pair"] for item in items if item["scalping_supported"]],"source":"INDODAX public ticker","note":"Market-data watchlist only; ranked by IDR volume and 24h price range position. It does not place trades."}

async def _serve_assets(scope,send):
    env=scope.get("env")
    if env is None or getattr(env,"ASSETS",None) is None: return False
    try:
        path=scope.get("path","/") or "/"; query=scope.get("query_string",b"").decode("latin-1")
        response=await env.ASSETS.fetch("https://assets.local"+path+(f"?{query}" if query else "")); body=await response.bytes()
        headers=[(str(k).lower().encode(),str(v).encode()) for k,v in dict(response.headers).items()]
        await _send(send,int(response.status),headers,body); return True
    except Exception: return False

async def _dashboard_status(scope):
    configured=bool(_env(scope,"SUPABASE_URL").strip() and _env(scope,"SUPABASE_SERVICE_ROLE_KEY").strip()); connected=await _supabase_probe(scope) if configured else False
    decisions=await _supabase_request(scope,"/rest/v1/decisions?select=action,created_at&order=created_at.desc&limit=500") or []
    buys=sum(1 for row in decisions if str(row.get("action","")).upper()=="BUY"); sells=sum(1 for row in decisions if str(row.get("action","")).upper()=="SELL"); holds=sum(1 for row in decisions if str(row.get("action","")).upper()=="HOLD")
    return {"portfolio_value":PAPER_INITIAL_BALANCE,"balance":PAPER_INITIAL_BALANCE,"daily_pnl":0.0,"daily_trades":0,"active_positions":0,"max_open_positions":MAX_OPEN_POSITIONS,"total_trades":0,"currency":"IDR","currency_symbol":"Rp","mode":"paper","bot_enabled":False,"cycle_running":False,"cycles_today":0,"last_cycle_at":None,"runtime_hours":0.0,"decision_counts":{"BUY":buys,"SELL":sells,"HOLD":holds},"database":{"configured":configured,"connected":connected,"status":"connected" if connected else ("not_configured" if not configured else "unreachable")},"market_data":{"source":"INDODAX public market data","available":True},"safety":{"mode":"paper","real_trading_locked":True,"bot_enabled":False}}

async def _dashboard_positions(scope):
    rows=await _supabase_request(scope,"/rest/v1/trades?status=eq.OPEN&select=symbol,action,entry_price,price,quantity,pnl,confidence,created_at&order=created_at.desc"); positions=[]
    for row in rows or []:
...
async def _dashboard_performance(scope):
    rows=await _supabase_request(scope,"/rest/v1/trades?select=pnl,status") or []; closed=[row for row in rows if row.get("status")=="CLOSED"]; pnl=sum(float(row.get("pnl") or 0) for row in closed); wins=sum(1 for row in closed if float(row.get("pnl") or 0)>0)
...
async def _dashboard_recent_decision(scope):
    ...
    if not rows:return {"decision":None}
    row=rows[0]
    return {"decision":{"id":row.get("id"),"symbol":row.get("symbol"),"action":row.get("action","HOLD"),"confidence":float(row.get("confidence") or 0)/100.0,"reasoning":row.get("reasoning"),"votes":row.get("agent_votes") or {},"market_scores":row.get("market_scores") or {},"confidence_components":row.get("confidence_components") or {},"consensus_action":row.get("consensus_action"),"consensus_score":row.get("consensus_score"),"position_size":row.get("position_size"),"stop_loss":row.get("stop_loss"),"take_profit":row.get("take_profit"),"engine_source":row.get("engine_source"),"engine_warning":row.get("engine_warning"),"created_at":row.get("created_at")}}
...
async def app(scope,receive,send):
    ...
    method=scope.get("method","GET").upper(); path=scope.get("path","/")
    if method=="OPTIONS": await _send(send,204,[],b""); return
    if not path.startswith("/api/"):
        if await _serve_assets(scope,send): return
        await _json_response(send,404,{"detail":"Not found"}); return
    if method=="GET" and path=="/api/health": await _json_response(send,200,{"status":"healthy","runtime":"cloudflare-python-worker"}); return
    if method=="GET" and path=="/api/ready":
        configured=all(_env(scope,name).strip() for name in ("SUPABASE_URL","SUPABASE_SERVICE_ROLE_KEY","JWT_SECRET_KEY","ADMIN_USERNAME","ADMIN_PASSWORD")); supabase=await _supabase_probe(scope) if configured else False
        await _json_response(send,200,{"status":"ready" if configured and supabase else "degraded","supabase":supabase,"secrets_configured":configured,"runtime":"cloudflare-python-worker"}); return
    if path=="/api/auth/login" and method=="POST":
        configured_user=_env(scope,"ADMIN_USERNAME").strip(); configured_password=_env(scope,"ADMIN_PASSWORD")
        if not configured_user or not configured_password: await _json_response(send,503,{"detail":"Dashboard authentication is not configured"}); return
        try: body=json.loads((await _read_body(receive)).decode("utf-8")); username=str(body.get("username","")); password=str(body.get("password",""))
        except Exception: await _json_response(send,400,{"detail":"Invalid JSON request body"}); return
        try: secret=_secret(scope); supplied=_hmac_sha256(secret,password); expected=_hmac_sha256(secret,configured_password)
        except RuntimeError as exc: await _json_response(send,503,{"detail":str(exc)}); return
        if username!=configured_user or not hmac.compare_digest(supplied,expected): await _json_response(send,401,{"detail":"Incorrect username or password"}); return
        await _json_response(send,200,{"access_token":_make_token(scope,configured_user),"token_type":"bearer","expires_in":1800,"username":configured_user}); return
