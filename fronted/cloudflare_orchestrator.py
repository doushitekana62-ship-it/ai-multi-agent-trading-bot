...
class CloudflareOrchestrator:
    ...
    def _market_features(cls, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Short windows are useful for scalping, but a missing 1m/5m window
        # must not zero-out a valid 15m/30m directional regime.
        momentum = cls._clip(
            (cls._number(m1) / 0.05) * 0.35
            + (cls._number(m5) / 0.15) * 0.25
            + (cls._number(m15) / 0.30) * 0.20
            + (cls._number(m30) / 0.60) * 0.20
        )
        ...
        # Direction is formed from independent directional evidence. Volume is
        # a confirmation signal, not a direction selector.
        directional = cls._clip(momentum * 0.55 + structure * 0.30 + pattern * 0.15)
        direction_sign = 1 if directional > 0.12 else -1 if directional < -0.12 else 0
        aligned_volume = abs(volume) >= 0.10
        confirmations = sum([
            ...
            bool(direction_sign and aligned_volume),
        ])
        combined = cls._clip(directional)
        opportunity = min(1.0, abs(combined) * 0.70 + min(confirmations / 3.0, 1.0) * 0.30)
        confidence = cls._clip(0.48 + abs(combined) * 0.22 + min(confirmations / 3.0, 1.0) * 0.22, 0.0, 0.86)
        return {
            "candles": candles,
            "move_1m_pct": m1,
            "move_5m_pct": m5,
    ...
    def _local_fallback(cls, symbol: str, market_data: Dict[str, Any], reason: str):
        ...
        confidence = features["confidence"] * 0.94
        ...
        votes = {
            ...
            "Volume/Pattern Analyst": "BUY" if features["pattern"] >= 0.20 else "SELL" if features["pattern"] <= -0.20 else "NEUTRAL",
        }
        ...
        details = {
            "momentum": {"score": features["momentum"], "status": "OK"},
            "structure": {"score": features["structure"], "status": "OK"},
            "decision": {"score": features["combined"], "status": "OK"},
            "forecast": {"score": features["move_30m_pct"] if features["move_30m_pct"] is not None else features["momentum"], "status": "OK"},
            "sentiment": {"score": features["volume"], "status": "OK"},
            "volume_pattern": {"score": features["pattern"], "status": "OK"},
        }
        data_available = bool(features["candles"])
        cycle_status = "AI_DEGRADED" if data_available else "DATA_UNAVAILABLE"
        hold_reason = None
        if action == "HOLD":
            hold_reason = "DATA_UNAVAILABLE" if not data_available else "NO_DIRECTIONAL_EDGE"
        ...
        return SimpleNamespace(
            ...
            market_scores={
                ...
                "forecast": features["move_30m_pct"] if features["move_30m_pct"] is not None else features["momentum"],
                "consensus": combined,
            },
            confidence_components={"opportunity_score": opportunity, "confirmations": float(confirmations), "data_quality": 1.0 if data_available else 0.0, "ai_degraded": 1.0},
            ...
            summary=f"Deterministic degraded analysis: {action}; score={combined:+.3f}; confirmations={confirmations}; pattern={features['pattern_name']}; m30={features['move_30m_pct']}; AI unavailable={reason}.",
            engine_source="deterministic_market_fallback",
            ...
            hold_analysis={"fallback": True, "reason": reason, "independent_evidence": votes, "confirmations": confirmations, "opportunity_score": opportunity, "data_available": data_available},
            knowledge_topics=["momentum", "market structure", "volume", "candlestick", "multi-timeframe regime"],
            candle_analysis={"available": data_available, "pattern": features["pattern_name"], "direction": "BUY" if features["pattern"] > 0 else "SELL" if features["pattern"] < 0 else "NEUTRAL"},
            library_alerts=[],
            library_version="deterministic-market-library-v3",
            cycle_status=cycle_status,
            agent_details=details,
            sentiment=SimpleNamespace(**details["sentiment"]),
            technical=SimpleNamespace(**details["structure"]),
            decision=SimpleNamespace(**details["decision"]),
            forecast=SimpleNamespace(**details["forecast"]),
            reflection=None,
            mimic_analysis=None,
        )
    ...
    async def analyze(self, symbol: str, market_data: Dict[str, Any] | None = None):
        ...
        try:
            ...
            def detail(name):
                return SimpleNamespace(**(details.get(name) or {}))
            return SimpleNamespace(
                ...
                engine_warning=None,
                hold_agents=list(data.get("hold_analysis", {}).get("hold_agents") or []),
                opposing_agents=list(data.get("hold_analysis", {}).get("opposing_agents") or []),
                hold_analysis=dict(data.get("hold_analysis") or {}),
                knowledge_topics=list(data.get("knowledge_topics") or []),
                candle_analysis=dict(data.get("candle_analysis") or {}),
                library_alerts=list(data.get("library_alerts") or [])[:4],
                library_version=str(data.get("library_version") or "unknown"),
                agent_details=details,
                cycle_status=str(data.get("cycle_status") or "ANALYZED"),
                sentiment=detail("sentiment"),
                technical=detail("technical"),
                decision=detail("decision"),
                forecast=detail("forecast"),
                reflection=detail("reflection"),
                mimic_analysis=detail("mimic_trader"),
            )
        except Exception as exc:
