...
remains functional when the external service is unavailable. The fallback is
fail-safe and never treats HOLD as a hidden veto over independent evidence.
"""
...
class CloudflareOrchestrator:
    ...
    def _local_fallback(cls, symbol: str, market_data: Dict[str, Any], reason: str):
        """Bounded paper-only ensemble used only when FastAPI Cloud is unavailable."""
        ...
        if price > 0 and action == "BUY":
            stop_loss, take_profit = price * 0.97, price * 1.03
        elif price > 0 and action == "SELL":
            stop_loss, take_profit = price * 1.03, price * 0.97
        ...
        return SimpleNamespace(
            ...
            consensus_score=directional_score, agent_votes=votes, market_scores={**scores, "consensus": directional_score},
            confidence_components={"directional_score": directional_score, "directional_strength": directional_strength,
                                   "hold_vote_count": float(len(hold_agents)), "directional_vote_count": float(len(directional_agents)),
                                   "opposing_vote_count": float(len(opposing_agents)), "fallback": 1.0},
            ...
            hold_agents=hold_agents, opposing_agents=opposing_agents,
            knowledge_topics=[], candle_analysis={"available": False, "pattern": "UNAVAILABLE", "direction": "NEUTRAL"},
            library_alerts=[], library_version="unavailable",
        )
    ...
    async def analyze(self, symbol: str, market_data: Dict[str, Any] | None = None):
        ...
        try:
            ...
            return SimpleNamespace(
                ...
                execution_reason=data.get("execution_reason"), hold_reason=data.get("hold_reason"),
                summary=data.get("summary") or "AI engine completed analysis.",
                engine_source="fastapi_cloud", engine_warning=None,
                hold_agents=[], opposing_agents=[],
