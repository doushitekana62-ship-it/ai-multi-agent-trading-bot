...
async def _get(path: str):
    try:
        response = await fetch(
            f"{BASE_URL}{path}",
            to_js({"method": "GET", "headers": {"Accept": "application/json", "Cache-Control": "no-cache", "User-Agent": "ai-trading-dashboard/1.0"}}),
        )
...
async def market_insights():
    ...
    for pair, ticker in raw.items():
        ...
        items.append({"pair": str(pair).upper().replace("_", "/"), "last": last, "volume_idr": volume, "high": high, "low": low, "range_position": round(range_position, 1), "signal": signal})
    items.sort(key=lambda item: item["volume_idr"], reverse=True)
    return {"items": items[:8], "source": "INDODAX public API", "available": True, "note": "Public ticker watchlist only; it does not place trades."}
