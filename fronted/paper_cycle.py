...
async def _persist_execution_trade(env, decision_id, cycle_id, session_id, symbol, action, confidence, price, execution_result):
    ...
    if side == "SELL":
        ...
        encoded_symbol = quote(symbol, safe=""); open_rows = await _supabase(env, "trades", method="GET", query=f"?select=*&symbol=eq.{encoded_symbol}&status=eq.OPEN&order=created_at.desc&limit=1"); rows = open_rows.get("rows") or []
        ...
        trade_id = rows[0].get("id"); patch = {"exit_price": _num(trade.get("price") or price), "price": _num(trade.get("price") or price), "pnl": _num(trade.get("pnl")), "status": "CLOSED", "closed_at": datetime.now(timezone.utc).isoformat(), "exit_decision_id": int(decision_id) if decision_id is not None else None}; updated = await _supabase(env, "trades", method="PATCH", query=f"?id=eq.{trade_id}", payload=patch)
...
def _normalize_trades(payload):
    rows = payload if isinstance(payload, list) else (payload.get("trades") or payload.get("data") or []) if isinstance(payload, dict) else []; out = []
    for item in rows:
        ...
        side = str(item.get("type") or item.get("side") or "").lower(); out.append({"tid": str(item.get("tid") or item.get("trade_id") or ""), "price": price, "timestamp": ts, "amount": _num(item.get("amount")), "type": side, "side": side, "source": str(item.get("source") or "INDODAX public market data"), "observation_type": str(item.get("observation_type") or ("TRADE" if item.get("tid") or item.get("trade_id") else "UNKNOWN")).upper()})
    return out
...
def _pulse(points, anchor):
    ...
    for point in points:
        ...
        bucket = int(ts // 60) * 60; row = buckets.setdefault(bucket, {"timestamp": datetime.fromtimestamp(bucket, timezone.utc).isoformat(), "open": price, "high": price, "low": price, "close": price, "trades": 0, "observations": 0, "changed": False, "last_direction": "GRAY"})
    ...
    for bucket in range(current - 29 * 60, current + 60, 60):
        ...
        move = ((row["close"] - row["open"]) / row["open"]) * 100.0 if row["open"] > 0 else 0.0; row["move_pct"] = move; row["status"] = "GREEN" if move > 0 else "RED" if move < 0 else row.get("last_direction", "GRAY") if row["changed"] else "GRAY"; segments.append(row)
    populated = [x for x in segments if x.get("observations", 0) > 0 and x.get("open")]
    if not populated: return segments, "GRAY", "GRAY", None
    net = ((populated[-1]["close"] - populated[0]["open"]) / populated[0]["open"]) * 100.0; overall = "GREEN" if net > 0 else "RED" if net < 0 else "GRAY"; return segments, overall, segments[-1].get("status", "GRAY"), net
...
def _ohlcv(points, anchor):
    ...
    for point in points:
        ...
        bucket = int(ts // 60) * 60; row = buckets.setdefault(bucket, {"timestamp": bucket * 1000, "open": price, "high": price, "low": price, "close": price, "volume": 0.0, "trades": 0}); row["high"] = max(row["high"], price); row["low"] = min(row["low"], price); row["close"] = price; row["volume"] += amount; row["trades"] += 1
...
async def _fetch_market(env, pair): return await cf_worker._market_overview({"env": env, "query_string": f"pair={pair}".encode("latin-1")})
...
async def run_paper_cycle(env, state_api, pair="btc_idr", state_response=None):
    ...
    try:
        ...
        execution_pass = risk_exit_override or (candidate in {"BUY", "SELL"} and confidence >= EXECUTION_CONFIDENCE_THRESHOLD and account_allows_candidate); action = candidate if execution_pass else "HOLD"; details = _agent_details(result); scores = {k: _num(v) for k, v in dict(getattr(result, "market_scores", {}) or {}).items()}; components = {k: _num(v) for k, v in dict(getattr(result, "confidence_components", {}) or {}).items()}; hold_analysis = dict(getattr(result, "hold_analysis", {}) or {})
