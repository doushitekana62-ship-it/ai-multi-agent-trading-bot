...
async def _persist_decision(env, symbol, action, confidence, reasoning, result):
    payload = {
        "symbol": str(symbol).upper(), "action": str(action).upper(),
...
async def _persist_trade(env, trade, decision_id=None):
    ...
    price = _json_number(trade.get("price")); qty = _json_number(trade.get("quantity"))
    payload = {
        "decision_id": decision_id, "symbol": trade.get("symbol"), "action": action,
        "price": price, "quantity": qty, "pnl": _json_number(trade.get("pnl")),
...
async def _persist_history(env, state, market, decision, metadata):
    ...
    payload = {
        "cycle_at": cycle_at, "trading_date": cycle_at[:10], "pair": pair,
        "action": str(decision.get("action") or "HOLD").upper(), "raw_action": metadata.get("raw_action"),
        "confidence": max(0, min(100, _json_number(decision.get("confidence")) * 100)),
        "price": _json_number(market.get("last")), "balance": _json_number(state.get("balance")),
        "portfolio_value": _json_number(state.get("portfolio_value")), "daily_pnl": _json_number(state.get("daily_pnl")),
        "total_pnl": _json_number(state.get("total_pnl")), "active_positions": int(state.get("active_positions", 0)),
        "positions": list(state.get("positions") or []), "agent_votes": metadata.get("votes") or {},
        "market_scores": metadata.get("market_scores") or {}, "confidence_components": metadata.get("confidence_components") or {},
        "consensus_action": metadata.get("consensus_action"), "consensus_score": _json_number(metadata.get("consensus_score")),
        "position_size": _json_number(metadata.get("position_size")), "stop_loss": _optional_json_number(metadata.get("stop_loss")),
        "take_profit": _optional_json_number(metadata.get("take_profit")), "reasoning": decision.get("reasoning"),
        "engine_source": metadata.get("source"), "engine_warning": metadata.get("warning"),
    }
    return await _post_supabase(env, "paper_history", payload)


def _reasoning_with_alert_bridge(reasoning, alerts):
    """Keep a bounded machine-readable alert bridge for the legacy paper-state schema."""
...
async def run_paper_cycle(env, state_api, pair="btc_idr", state_response=None):
    ...
    try:
        ...
        # Keep a small trade history for the dashboard and let the FastAPI engine
        # aggregate it into true time-bucketed OHLCV candles.
        ...
        market_data = {
            ...
            "short_term_move_percent": effective_move, "public_trade_move_percent": public_move,
            "window_move_percent": window_move, "timeframe": "1m-trades", "ohlcv": [],
            "recent_trades": points, "volatility": None,
            "data_quality_score": 0.85 if len(history) >= 80 else max(0.45, min(0.80, len(history) / 100.0)),
        ...
        orchestrator = CloudflareOrchestrator(
            {"min_confidence": EXECUTION_CONFIDENCE_THRESHOLD, "max_position_size": MAX_POSITION_SIZE, "debug_enabled": True}, env=env
        )
        ...
        metadata = {
            ...
            "agents_invoked": list(dict(result.agent_votes or {}).keys()), "hold_agents": hold_agents, "opposing_agents": opposing_agents,
            "market_history_points": len(history), "public_trade_move_percent": public_move, "window_move_percent": window_move,
            "effective_move_percent": effective_move, "range_position": range_pos, "created_at": result.timestamp.isoformat(),
        ...
        bridged_reasoning = _reasoning_with_alert_bridge(reasoning, library_alerts)
        persistence = await _persist_decision(env, symbol, action, confidence, bridged_reasoning, result)
        ...
        decision = state.get("last_decision") or {"action": action, "confidence": confidence, "reasoning": bridged_reasoning}
        ...
        return {
            ...
            "market": market, "orchestrator": metadata,
