...
async def _persist_market_observation(env,payload):
    ...
    if not base or not key:return {"ok":False,"saved":False,"reason":"supabase_credentials_missing"}
    try:
        opts={"method":"POST","headers":{"apikey":key,"Authorization":f"Bearer {key}","Accept":"application/json","Content-Type":"application/json"},"body":json.dumps(_safe(payload),separators=(",",":"))}
...
async def run_paper_cycle(env,state_api,pair="btc_idr",state_response=None):
    ...
    try:
        ...
        action=str(getattr(result,"final_action","HOLD") or "HOLD").upper(); confidence=max(0,min(1,_num(getattr(result,"final_confidence",0)))); price=_num(market.get("last")); pulse=getattr(result,"candle_analysis",{}).get("pulse",{}) if isinstance(getattr(result,"candle_analysis",{}),dict) else {}; pulse_segments=pulse.get("segments") or []
        execution_status="NOT_EXECUTED"; risk_rejection=None; trade_id=None
        before=len(state.get("trade_history") or []); state=await state_api.record_cycle(action,confidence,market["pair"].upper().replace("_","/"),price,getattr(result,"summary","") or "",{"votes":votes,"market_scores":scores,"confidence_components":getattr(result,"confidence_components",{}),"consensus_action":getattr(result,"consensus_action","HOLD"),"consensus_score":getattr(result,"consensus_score",0),"position_size":getattr(result,"position_size",0),"source":getattr(result,"engine_source","indodax_native"),"warning":getattr(result,"engine_warning",None),"raw_action":action,"summary":getattr(result,"summary","") ,"cycle_id":cycle_id,"cycle_number":int(state.get("cycles_today",0))+1,"market_timestamp":datetime.fromtimestamp(anchor,timezone.utc).isoformat(),"market_source":"INDODAX public market data","move_1m_pct":_latest_move(history,1,anchor),"move_5m_pct":_latest_move(history,5,anchor),"move_15m_pct":_latest_move(history,15,anchor),"move_30m_pct":_latest_move(history,30,anchor),"pulse_status":pulse.get("overall") or getattr(result,"consensus_action","HOLD"),"current_pulse_status":pulse.get("current") or "GRAY","pulse_net_move_30m_pct":pulse.get("net_move_pct"),"pulse_segments":pulse_segments,"candidate_action":getattr(result,"consensus_action",action),"cycle_status":"ANALYZED","hold_analysis":getattr(result,"hold_analysis",{}),"symbol":market["pair"].upper().replace("_","/")})
