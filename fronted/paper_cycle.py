...
async def _persist_market_observation(env,payload):
    ...
    if not base or not key:return {"ok":False,"saved":False,"reason":"supabase_credentials_missing"}
    try:
        opts={"method":"POST","headers":{"apikey":key,"Authorization":f"Bearer {key}","Accept":"application/json","Content-Type":"application/json"},"body":json.dumps({"p_payload":_safe(payload)},separators=(",",":"))}
...
async def run_market_observation(env,state_api,pair="btc_idr",session_id=None):
    ...
    minute=datetime.fromtimestamp(int(anchor//60)*60,timezone.utc).isoformat();pulse_status="GRAY";changed=False;last_direction=None;current_rows=[x for x in history if int(_ts(x)//60)==int(anchor//60)]
    if current_rows:
        ordered=sorted(current_rows,key=_ts);o=_num(ordered[0].get("price"));c=_num(ordered[-1].get("price"));changed=any(_num(ordered[i].get("price"))!=_num(ordered[i-1].get("price")) for i in range(1,len(ordered)));last_direction=None
        for i in range(len(ordered)-1,0,-1):
            a,b=_num(ordered[i-1].get("price")),_num(ordered[i].get("price"))
            if b!=a:last_direction="GREEN" if b>a else "RED";break
        pulse_status="GREEN" if c>o else "RED" if c<o else last_direction or "GRAY"
