...
function App() {
  ...
  const theme = createTheme({
    ...
    components: { MuiAppBar: { styleOverrides: { root: { background: mode === 'dark' ? 'linear-gradient(90deg, #0d1820 0%, #122b2b 55%, #0d1820 100%)' : 'linear-gradient(90deg, #17353b 0%, #24504f 55%, #17353b 100%)', color: '#f3f8f9', borderBottom: '1px solid', borderColor: mode === 'dark' ? 'rgba(34,211,167,.22)' : 'rgba(34,211,167,.28)', boxShadow: mode === 'dark' ? '0 8px 30px rgba(0,0,0,.30)' : '0 8px 26px rgba(22,55,63,.16)' } } }, MuiPaper: { styleOverrides: { root: { border: '1px solid', borderColor: mode === 'dark' ? 'rgba(159,189,201,.10)' : 'rgba(38,64,74,.10)', backgroundImage: 'none', boxShadow: mode === 'dark' ? '0 12px 34px rgba(0,0,0,.20)' : '0 10px 30px rgba(22,55,63,.07)' } } }, MuiCard: { styleOverrides: { root: { border: '1px solid', borderColor: mode === 'dark' ? 'rgba(159,189,201,.10)' : 'rgba(38,64,74,.10)', backgroundImage: 'none', transition: 'transform .18s ease, box-shadow .18s ease, border-color .18s ease', '&:hover': { transform: 'translateY(-2px)', boxShadow: mode === 'dark' ? '0 14px 30px rgba(0,0,0,.28)' : '0 14px 30px rgba(22,55,63,.11)', borderColor: mode === 'dark' ? 'rgba(34,211,167,.32)' : 'rgba(8,127,106,.28)' } } } }, MuiButton: { styleOverrides: { root: { borderRadius: 9, textTransform: 'none', fontWeight: 700 } } }, MuiToggleButton: { styleOverrides: { root: { borderRadius: 8, textTransform: 'none', fontWeight: 700 } } }, MuiChip: { styleOverrides: { root: { fontWeight: 600 } } } },
  });
  return <ThemeProvider theme={theme}><CssBaseline /><AuthProvider><Router><Routes><Route path="/login" element={<Login />} /><Route path="/" element={<ProtectedRoute><DashboardLanding /></ProtectedRoute>} /><Route path="/reports" element={<ProtectedRoute><Reports /></ProtectedRoute>} /></Routes><CycleUpdateToast /><TradingLibraryAlertToast /><Box sx={{ position: 'fixed', right: 18, bottom: 18, zIndex: 1400 }}><Tooltip title={mode === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'}><IconButton onClick={toggleMode} aria-label="toggle dashboard theme" sx={{ width: 46, height: 46, color: 'primary.main', bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', boxShadow: 4, '&:hover': { bgcolor: 'action.hover' } }}>{mode === 'dark' ? <LightMode /> : <DarkMode />}</IconButton></Tooltip></Box></Router><Toaster position="top-right" toastOptions={{ style: { background: mode === 'dark' ? '#121c24' : '#f9fbfc', color: mode === 'dark' ? '#e8f0f3' : '#17242b' } }} /></AuthProvider></ThemeProvider>;
}
