...
import { Box, Card, CardContent, Chip, Divider, FormControl, Grid, InputLabel, MenuItem, Select, Stack, Typography } from '@mui/material';
...
const normalizedAgentKey = (name) => String(name || '').trim().toLowerCase().replace(/\s+/g, ' ');
const agentWeight = (name) => {
  const key = normalizedAgentKey(name);
  if (key.includes('sentiment')) return 0.08;
  if (key.includes('technical')) return 0.34;
  if (key.includes('decision')) return 0.28;
  if (key.includes('forecast')) return 0.18;
  if (key.includes('mimic')) return 0.12;
  return 0;
};
...
function ConflictAnalyzer({ decision }) {
  ...
  const influence = blockers.map((name) => ({ name, weight: agentWeight(name), score: Math.abs(Number(scores[normalizedAgentKey(name).replace(' agent', '')] ?? scores[normalizedAgentKey(name)] ?? 0)) })).sort((a, b) => (b.weight * b.score) - (a.weight * a.score));
  const meaningfulBlockers = influence.filter((item) => item.weight > 0 && item.score > 0.01);
  const conflict = (directional.length > 0 && blockers.length > 0) || opposingAgents.length > 0;
  const dominant = meaningfulBlockers[0]?.name || (directional.length ? 'No directional blocker' : 'No meaningful blocker');
  return <Card variant="outlined" sx={{ height: '100%' }}><CardContent><Stack direction="row" justifyContent="space-between" alignItems="center"><Box><Typography variant="h6">Conflict Analyzer</Typography><Typography variant="caption" color="text.secondary">Why agents agree, HOLD, or oppose the candidate.</Typography></Box><Chip size="small" label={conflict ? 'CONFLICT' : 'ALIGNED'} color={conflict ? 'warning' : 'success'} /></Stack><Stack spacing={1.2} sx={{ mt: 2 }}><Stack direction="row" justifyContent="space-between"><Typography variant="body2">Final action</Typography><Chip size="small" label={action} color={action === 'BUY' ? 'success' : action === 'SELL' ? 'error' : 'default'} /></Stack><Stack direction="row" justifyContent="space-between"><Typography variant="body2">Directional agents</Typography><Typography fontWeight={700}>{directional.length}</Typography></Stack><Stack direction="row" justifyContent="space-between"><Typography variant="body2">Meaningful HOLD blockers</Typography><Typography fontWeight={700}>{meaningfulBlockers.length}</Typography></Stack><Stack direction="row" justifyContent="space-between"><Typography variant="body2">Opposing agents</Typography><Typography fontWeight={700}>{opposingAgents.length}</Typography></Stack><Divider /><Typography variant="caption" color="text.secondary">Dominant meaningful blocker</Typography><Typography variant="body2" fontWeight={700}>{dominant}</Typography><Typography variant="caption" color="text.secondary">Score map: technical {Number(scores.technical || 0).toFixed(2)} · decision {Number(scores.decision || 0).toFixed(2)} · forecast {Number(scores.forecast || 0).toFixed(2)} · sentiment {Number(scores.sentiment || 0).toFixed(2)}</Typography></Stack></CardContent></Card>;
}
