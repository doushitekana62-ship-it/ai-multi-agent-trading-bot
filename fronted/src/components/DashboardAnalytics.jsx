...
function VolumeShare({ insights }) {
  ...
  return <Card variant="outlined" sx={{ height: '100%' }}><CardContent><Typography variant="h6">Volume Share</Typography><Typography variant="caption" color="text.secondary">Share of the displayed scanner volume by IDR.</Typography><Box sx={{ height: 300, mt: 1 }}>{rows.length ? <Doughnut data={data} options={options} /> : <Box sx={{ height: '100%', display: 'grid', placeItems: 'center', color: 'text.secondary' }}>No volume data.</Box>}</Box></CardContent></Card>;
}
