...
const POSITION_PREF_KEY = 'paperTradingMaxPositions';
...
export default function DashboardTools({ market, decision, counts, runtimeHours = 0, cycles = 0, targets, dailyActual = 0, decisionHistory = [], positions = [] }) {
  ...
  const [maxPositions, setMaxPositions] = useState(() => {
    try {
      const saved = Number(localStorage.getItem(POSITION_PREF_KEY));
      return saved >= 1 && saved <= 3 ? saved : 3;
    } catch { return 3; }
  });
  ...
  const positionUserChanged = useRef(false);

  const loadHealth = async () => {
    try {
      const response = await axios.get('/api/dashboard/status', { params: { _ts: Date.now() }, headers: { 'Cache-Control': 'no-cache' } });
      setHealth(response.data?.system_health || null);
      const serverValue = Number(response.data?.max_open_positions);
      if (!positionUserChanged.current && Number.isFinite(serverValue) && serverValue >= 1 && serverValue <= 3) {
        const saved = Number(localStorage.getItem(POSITION_PREF_KEY));
        if (!(saved >= 1 && saved <= 3)) setMaxPositions(serverValue);
      }
  ...
  const loadHistory = async (date = historyDate) => {
    ...
    try {
      const response = await axios.get('/api/dashboard/history', { params: { date, _ts: Date.now() }, headers: { 'Cache-Control': 'no-cache' } });
      setHistory(response.data?.history || []);
    } catch (error) {
      console.error('Unable to fetch paper history:', error);
      setHistory([]);
      const detail = error.response?.data?.detail;
      const status = error.response?.status;
      setHistoryError(detail || (status === 404 ? 'Paper history route is not available on the deployed worker. Redeploy the latest Cloudflare worker.' : 'History unavailable'));
  ...
  const saveMaxPositions = async (event) => {
    ...
    const previous = maxPositions;
    positionUserChanged.current = true;
    setMaxPositions(value);
    try {
      const response = await axios.post('/api/dashboard/paper/settings', { max_open_positions: value });
      const confirmed = Number(response.data?.max_open_positions);
      const finalValue = confirmed >= 1 && confirmed <= 3 ? confirmed : value;
      setMaxPositions(finalValue);
      localStorage.setItem(POSITION_PREF_KEY, String(finalValue));
    } catch (error) {
      console.error('Unable to save position limit:', error);
      setMaxPositions(previous);
      try { localStorage.setItem(POSITION_PREF_KEY, String(previous)); } catch {}
    }
  };
