...
def _make_refresh_token(env, username: str) -> str:
    ...
    return f"{raw}.{cf_worker._b64url(sig)}"
...
def _runtime_hours(started_at):
    ...
    try:
        start = time.mktime(time.strptime(str(started_at)[:19], "%Y-%m-%dT%H:%M:%S"))
        return max(0.0, (time.time() - start) / 3600.0)
    except (TypeError, ValueError, OverflowError): return 0.0


def _state_response(state):
    counts = dict(state.get("decision_counts") or {}); enabled = bool(state.get("enabled")); running = bool(state.get("cycle_running"))
    return {
        **state,
        "bot_enabled": enabled, "enabled": enabled, "cycle_running": running, "mode": "paper",
        ...
        "decision_counts": {k:int(counts.get(k,0)) for k in ("BUY","SELL","HOLD")},
        "safety": {"mode":"paper", "real_trading_locked":True, "bot_enabled":enabled, "cycle_running":running},
    }


def _request_pair(request, default="btc_idr"):
    try:
        q = parse_qs(urlparse(request.url).query); value = q.get("pair", q.get("symbol", [default]))[0]
        return cf_worker._clean_pair(value)
    except Exception: return default


def _supabase_config(env):
...
async def _history(env, request):
    ...
    try:
        ...
        return Response.json({"history":rows if isinstance(rows,list) else [],"count":len(rows) if isinstance(rows,list) else 0,"date":date or None,"pair":pair.upper() if pair else None,"connected":True})
    except Exception as exc:
        return Response.json({"history":[],"connected":False,"reason":type(exc).__name__}, status=502)
...
class Default(WorkerEntrypoint):
    ...
    async def _handle_auth(self, request, path):
        ...
        if path == "/api/auth/refresh" and request.method == "POST":
            try: body = await request.json(); refresh = str(body.get("refresh_token",""))
            except Exception: return Response.json({"detail":"Invalid JSON request body"}, status=400)
            payload = cf_worker._verify_token({"env":self.env}, refresh); user = str(getattr(self.env,"ADMIN_USERNAME","") or "").strip()
        ...
        if path == "/api/auth/verify" and request.method == "GET":
            ...
            return Response.json({"username":payload.get("sub"),"is_authenticated":True}) if payload else Response.json({"detail":"Invalid or expired token"}, status=401)
    ...
    async def _handle_state_routes(self, request, path):
        ...
        if protected and not await self._verify_access(request): return Response.json({"detail":"Invalid or expired token"}, status=401)
        stub = await _state_stub(self.env)
        if path in ("/api/bot/status", "/api/dashboard/status") and request.method == "GET":
            state = await stub.get_state(); result = _state_response(state)
            health = await _supabase_health(self.env)
            market = await cf_worker._market_overview({"env":self.env,"query_string":f"pair={state.get('paper_pair','btc_idr')}".encode("latin-1")})
            result.update({"daily_pnl":float(state.get("daily_pnl",0)),"daily_trades":int(state.get("daily_trades",0)),"total_trades":int(state.get("total_trades",0)),"active_positions":len(state.get("positions") or []),"database":{"configured":bool(_supabase_config(self.env)[0] and _supabase_config(self.env)[1]),"connected":bool(health.get("connected")),"status":"connected" if health.get("connected") else "unavailable"},"market_data":{"source":"INDODAX public market data","available":bool(market.get("available")),"fresh":bool(market.get("available")),"stale":not bool(market.get("available")),"age_seconds":0 if market.get("available") else None},"system_health":{"database":{"connected":bool(health.get("connected"))},"market_data":{"fresh":bool(market.get("available")),"stale":not bool(market.get("available")),"age_seconds":0 if market.get("available") else None},"mode":"paper","engine":{"running":bool(state.get("cycle_running")),"enabled":bool(state.get("enabled"))}}})
            return Response.json(result)
        if path in ("/api/bot/start", "/api/dashboard/paper/start") and request.method == "POST":
            state = await stub.start(_request_pair(request)); cycle = await run_paper_cycle(self.env, stub, _request_pair(request))
            return Response.json({**_state_response(cycle.get("state") or state),"message":"Paper trading started and one AI cycle completed.","cycle":cycle})
        ...
        if path == "/api/dashboard/paper/settings" and request.method == "POST":
            ...
            state = await stub.set_position_limit(value); return Response.json(_state_response(state))
        ...
        if path == "/api/dashboard/performance" and request.method == "GET":
            state = await stub.get_state(); history = list(state.get("trade_history") or []); closed = [t for t in history if t.get("action") == "SELL"]; wins = sum(1 for t in closed if float(t.get("pnl") or 0)>0)
            return Response.json({"performance":{"total_pnl":float(state.get("total_pnl",0)),"daily_pnl":float(state.get("daily_pnl",0)),"closed_trades":len(closed),"win_rate":wins/len(closed) if closed else 0.0,"balance":float(state.get("balance",0)),"portfolio_value":float(state.get("portfolio_value",0)),"total_trades":int(state.get("total_trades",0)),"currency":"IDR","currency_symbol":"Rp"}})
        ...
        if path == "/api/dashboard/agents" and request.method == "GET":
            state = await stub.get_state(); status = "armed" if state.get("enabled") else "idle"
            return Response.json({"agents":[{"name":name,"status":status,"description":"Five-agent paper execution pipeline."} for name in ["Sentiment Agent","Technical Agent","Decision Agent","Forecast Agent","Reflector Agent"]]})
    ...
    async def fetch(self, request):
        ...
        try:
            ...
            print(f"[worker:fetch] error path={path} type={type(exc).__name__}: {exc}")
