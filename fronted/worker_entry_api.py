...
class Default(BaseDefault):
    async def _handle_state_routes(self, request, path):
        ...
        if target == "/api/bot/start" and request.method == "POST":
            ...
            try:
                ...
                state = await stub.enable_paper(_request_pair(request))
        ...
        if target == "/api/bot/stop" and request.method == "POST":
            if not await self._verify_access(request):
                return Response.json({"detail": "Invalid or expired token"}, status=401)
            try:
                state = await (await _state_stub(self.env)).stop()
                payload = _state_response(state)
        ...
        if target == "/api/dashboard/status" and request.method == "GET":
            ...
            result.update({
                ...
                "market_data": {"source": "INDODAX public API", "available": bool(market.get("available")), "fresh": bool(market.get("available")), "stale": not bool(market.get("available")), "pair": market.get("pair"), "last": market.get("last"), "recent_move": market.get("recent_move")},
                "manual_control": True,
                "automation_enabled": True,
                "cycle_schedule": "durable_object_alarm_1m",
                "scheduler": {"active": bool(state.get("scheduler_active")) and bool(state.get("enabled")), "source": state.get("scheduler_source", "durable_object_alarm"), "last_run_at": state.get("last_scheduler_at"), "next_run_at": state.get("next_cycle_at"), "invocations": int(state.get("scheduler_invocations", 0))},
                "cycle": {"number": int(state.get("cycles_today", 0)), "status": state.get("last_cycle_status", "idle"), "last_started_at": state.get("last_cycle_started_at"), "last_finished_at": state.get("last_cycle_finished_at"), "last_cycle_at": state.get("last_cycle_at"), "failures": int(state.get("cycle_failures", 0)), "consecutive_failures": int(state.get("consecutive_cycle_failures", 0)), "last_error": state.get("last_error")},
                "system_health": {"database": {"connected": True, "diagnostic": "durable_object_state"}, "market_data": {"fresh": bool(market.get("available")), "stale": not bool(market.get("available"))}, "mode": "paper", "engine": {"running": bool(state.get("enabled")), "enabled": bool(state.get("enabled")), "cycle_running": bool(state.get("cycle_running")), "state": "RUNNING" if state.get("enabled") else "OFF", "last_cycle_status": state.get("last_cycle_status", "idle"), "last_error": state.get("last_error")}},
            })
        ...
        if target == "/api/dashboard/agents" and request.method == "GET":
            ...
            status = "armed" if state.get("enabled") else "idle"
    ...
    async def scheduled(self, controller, env, ctx):
        """Legacy compatibility hook; Durable Object Alarm is authoritative."""
