...
create table if not exists public.bot_health (id uuid primary key default gen_random_uuid(),user_id uuid not null references auth.users(id) on delete cascade,state text not null,data_fresh boolean not null default true,api_latency_ms numeric,heartbeat_at timestamptz not null default now());
