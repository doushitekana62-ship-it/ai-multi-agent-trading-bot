...
def test_worker_uses_shared_cycle_runner_and_risk_api():
    ...
    assert "from paper_cycle import run_paper_cycle" in source;assert "await run_paper_cycle(" in source;assert "await stub.enable_paper(pair)" in source;assert "await stub.stop()" in source;assert "/api/dashboard/paper/risk" in source;assert "risk_settings" in source
...
def test_ai_engine_failure_is_explicitly_degraded_not_silent_hold():
    adapter=(FRONTED/"cloudflare_orchestrator.py").read_text(encoding="utf-8");assert "AI_DEGRADED" in adapter;assert 'cycle_status="AI_DEGRADED" if engine_warning else "ANALYZED"' in adapter;assert 'data["engine_warning"]' in adapter
...
def test_risk_engine_supports_adaptive_and_fixed_modes():
    source=(FRONTED/"risk_engine.py").read_text(encoding="utf-8");assert "FIXED_PERCENT" in source;assert "ATR" in source;assert "RISK_REWARD" in source;assert "TRAILING_STOP" in source or "trailing_enabled" in source;assert "BREAK_EVEN" in source or "break_even_enabled" in source;assert "slippage_bps" in source
