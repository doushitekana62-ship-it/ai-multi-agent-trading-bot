...
def test_market_pulse_marks_intraminute_reversal_as_directional():
    now = 10 * 60 + 2
    rows = [{"timestamp": now - 1.5, "price":100.0},{"timestamp": now - 1.0, "price":101.0},{"timestamp": now - 0.5, "price":100.0}]
...
def test_market_pulse_keeps_gray_only_for_flat_or_empty_minutes():
    now = 10 * 60 + 2
    rows = [{"timestamp":now - 1.5,"price":100.0},{"timestamp":now - 0.5,"price":100.0}]
    pulse = _pulse(rows, now)
