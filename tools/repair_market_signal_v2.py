...
# Existing deterministic syntax repair.
...
# The dashboard forecast must be a forward-looking observation, not a second
# historical price series. Previously every historical point was multiplied by
# the current forecast score, so a stale/negative/positive score produced a
# visually dramatic line even before the paper bot was running.
replace_if_present(
    dashboard,
...
# Paper history previously stored the pre-execution account snapshot and only
# afterwards mutated the authoritative Durable Object ledger. Reconcile the
# just-written decision/history rows with the post-execution ledger so the
# dashboard cannot show a balance/equity state that predates the displayed BUY.
...
# Make execution visible in the forensic history card: action and execution
# are not the same thing. BUY/HOLD/SELL is the decision; FILLED is the actual
